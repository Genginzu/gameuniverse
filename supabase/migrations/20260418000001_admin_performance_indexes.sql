...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_game_translations_title_trgm
  ON public.game_translations USING gin (title gin_trgm_ops);

-- ── character_translations.name: admin characters search ──
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_character_translations_name_trgm
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_ratings_rating_system_id
  ON public.ratings (rating_system_id);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_content_descriptors_rating_system_id
  ON public.content_descriptors (rating_system_id);

-- ── character_games.is_primary filter for admin characters list ──
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_character_games_primary
