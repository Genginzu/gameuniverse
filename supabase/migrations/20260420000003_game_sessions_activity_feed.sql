-- Migration: expose game_sessions in the player activity feed.
-- 1) Add created_at column to game_sessions (started_at can be backdated by the
--    user — we need a separate insertion timestamp for feed ordering).
-- 2) Extend get_player_activity() with a 7th UNION ALL branch that projects
--    game_sessions rows as "session" activity events.

-- ========================================
-- 1. game_sessions.created_at
-- ========================================
...
CREATE INDEX IF NOT EXISTS idx_game_sessions_created_at_desc
...
COMMENT ON COLUMN public.game_sessions.created_at IS 'Timestamp when the session was logged (distinct from started_at, which can be backdated)';

-- ========================================
-- 2. get_player_activity — add "session" branch
-- ========================================
...
BEGIN
  ...
  WITH activity AS (
    ...
    INNER JOIN public.games g ON g.id = gs.game_id
    LEFT JOIN public.game_translations gt_loc
      ON gt_loc.game_id = g.id AND gt_loc.language_code = locale_code
    LEFT JOIN LATERAL (
      SELECT gt2.title
...
COMMENT ON FUNCTION get_player_activity(UUID, TEXT, TEXT, INT, INT)
  IS 'Aggregates player activity from game_reviews, character_comments, user_library, character_favorites, game_collections and game_sessions into a paginated JSONB feed sorted by date descending. Supports optional filtering by event type and locale-aware game/character names.';
