/**
 * Age ratings (PEGI, ESRB, etc.) for games.
 * Deno port of src/lib/services/game-import/age-ratings.ts.
 */
...
export async function createAgeRatings(gameId: string, igdbGame: IGDBGame): Promise<void> {
  ...
  if (ageRatings.length === 0) return;

  const supabase = getSupabaseAdmin();

  // Sort: PEGI first (will become primary)
  ageRatings.sort((a, b) => {
  ...
  for (let i = 0; i < ageRatings.length; i++) {
    ...
    if (organization === undefined || ratingCategory === undefined) continue;
