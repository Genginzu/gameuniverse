/**
 * Language support entries for games.
 * Deno port of src/lib/services/game-import/languages.ts.
 */
...
export async function createLanguages(gameId: string, igdbGame: IGDBGame): Promise<void> {
  ...
  // Group language supports by language code (consolidate audio/subs/interface)
  ...
  const nameMap = new Map(
    (supportedLangs ?? []).map((sl: { code: string; name: string }) => [sl.code, sl.name]),
  );
