/**
 * Applies an IGDB webhook update payload to a local game while
...
export async function applyWebhookPayload(
  ...
  if (payload.first_release_date !== undefined) {
    if (can("release_date", ov, f)) {
      gameUpdate.release_date = payload.first_release_date
        ? new Date((payload.first_release_date as number) * 1000)
            .toISOString()
            .split("T")[0]
        : null;
      applied.push("release_date");
    } else {
      skipped.push("release_date");
    }
  }
  ...
  if (payload.aggregated_rating !== undefined) {
    if (can("metascore", ov, f)) {
      ...
      skipped.push("metascore");
    }
  }

  // IGDB webhooks send sub-entity fields (cover/artworks/screenshots/videos)
  // as raw IDs instead of expanded objects. We resolve them via the IGDB API
  // on demand and cache the lookup for the duration of this call.
  ...
  if (resolvedArtworks !== undefined || resolvedScreenshots !== undefined) {
    if (can("background_image", ov, f)) {
      ...
      if (bgUrl) {
        await supabase
          .from("games")
          .update({ background_image_url: bgUrl })
          .eq("id", gameId);
        applied.push("background_image");
      }
    } else {
      skipped.push("background_image");
    }
  }
  ...
  if (resolvedScreenshots !== undefined) {
    if (can("screenshots", ov, f)) {
      ...
      if (items.length > 0) {
        await untypedTable(supabase, "game_screenshots").delete().eq("game_id", gameId);
        await untypedTable(supabase, "game_screenshots").insert(items);
        applied.push("screenshots");
      }
    } else {
      skipped.push("screenshots");
    }
  }
  ...
  if (resolvedArtworks !== undefined) {
    if (can("artworks", ov, f)) {
      ...
      if (items.length > 0) {
        await untypedTable(supabase, "game_artwork").delete().eq("game_id", gameId);
        await untypedTable(supabase, "game_artwork").insert(items);
        applied.push("artworks");
      }
    } else {
      skipped.push("artworks");
    }
  }
  ...
  if (rawVideos !== undefined && Array.isArray(rawVideos)) {
    if (can("videos", ov, f)) {
      ...
      if (resolvedVideos?.length) {
        ...
        if (items.length > 0) {
          await untypedTable(supabase, "game_videos").delete().eq("game_id", gameId);
          await untypedTable(supabase, "game_videos").insert(items);
          applied.push("videos");
        }
      }
    } else {
  ...
  // Genres
  const pGenres = payload.genres as
          ...
          if (existing) {
            ...
            if (created) {
              ...
              await supabase
                ...
                .insert({ genre_id: id, language_code: "en", name: g.name });
            }
          }
        }
  ...
  // Platforms
  const pPlatforms = payload.platforms as
    ...
    if (can("platforms", ov, f)) {
      ...
      if (valid.length > 0) {
        ...
        for (const p of valid) {
          const { data: existing } = await untypedTable(supabase, "platforms")
            .select("id")
            .eq("igdb_id", p.id)
            .single();
          if (existing) {
            ...
            const slug = (p.name || `platform-${p.id}`)
              ...
              .replace(/-+/g, "-")
              .replace(/^-|-$/g, "");
            const { data: created } = await untypedTable(supabase, "platforms")
            ...
            if (created) {
              const id = created.id as string;
              platformIds.push(id);
              await untypedTable(supabase, "platform_translations").insert({
                platform_id: id,
                language_code: "en",
                name: p.name,
              });
        ...
        if (platformIds.length > 0) {
          await untypedTable(supabase, "game_platforms")
            .delete()
            .eq("game_id", gameId);
          await untypedTable(supabase, "game_platforms").insert(
  ...
  if (pCompanies !== undefined) {
    if (can("companies", ov, f)) {
      const valid = pCompanies.filter(
        (c) => typeof c === "object" && c?.company?.slug,
      );
      if (valid.length > 0) {
        const rows: Array<{
          game_id: string;
        ...
        for (const ic of valid) {
          ...
          if (existing) {
            ...
            const { data: created } = await supabase
              ...
              .select("id")
              .single();
            if (created) companyId = created.id as string;
          }
  ...
  // Age ratings: complex — we don't auto-apply (need full import logic).
  const pAgeRatings = payload.age_ratings as
    | Array<{ rating_category: number }>
    | undefined;
  if (pAgeRatings !== undefined && pAgeRatings.length > 0) {
    if (can("age_ratings", ov, f)) {
      logger.info("Webhook: age_ratings present but require full import logic", {
        gameId,
      });
    } else {
    ...
    if (can("similar_games", ov, f)) {
      if (pSimilar.length > 0) {
        ...
        const igdbToLocal = new Map(
          (localGames ?? []).map(
            (g: { id: string; igdb_id: number }) =>
              [g.igdb_id, g.id] as [number, string],
          ),
        );
        ...
        await untypedTable(supabase, "game_similar_games")
          .delete()
          .eq("game_id", gameId);
        await untypedTable(supabase, "game_similar_games").upsert(rows, {
          onConflict: "game_id,similar_igdb_id",
        });
        applied.push("similar_games");
      }
    } else {
      skipped.push("similar_games");
    }
  }
