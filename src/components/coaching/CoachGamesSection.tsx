...
export function CoachGamesSection() {
  ...
  const { data, isLoading, mutate } = useSWR<{ games: CoachGame[] }>(
    "/api/coaching/games",
    fetcher
  );
  ...
  const [searchResults, setSearchResults] = useState<
    Array<{ id: string; title: string; coverImage: string | null }>
  >([]);
  ...
  const searchGames = async (q: string) => {
    ...
    setSearchResults(
      json.games?.map((g: { id: string; title: string; coverImage: string | null }) => ({
        id: g.id,
        title: g.title,
  ...
  const addGame = async (gameId: string) => {
    await fetch("/api/coaching/games", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ gameId, rankLevel: "", hoursExperience: 0, specialties: [] }),
    });
    setAdding(false);
    setSearch("");
    setSearchResults([]);
  ...
  const toggleSpecialty = async (game: CoachGame, specialty: string) => {
    const newSpecialties = game.specialties.includes(specialty)
      ? game.specialties.filter((s) => s !== specialty)
      : [...game.specialties, specialty];
    // Optimistic update
    mutate((prev: { games: CoachGame[] } | undefined) => {
      if (!prev) return prev;
      return {
        games: prev.games.map((g) =>
          g.id === game.id ? { ...g, specialties: newSpecialties } : g
        ),
      };
    }, false);
    fetch(`/api/coaching/games/${game.id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="from-palette-secondary-500 to-palette-primary-500 flex items-center gap-1.5 rounded-lg bg-linear-to-r px-3 py-1.5 text-xs font-medium text-white"
        >
        ...
        <div className="border border-editorial-line bg-editorial-2 space-y-2 rounded-xl p-4">
          ...
          {searchResults.map((game) => (
            <button
              ...
              className="flex w-full items-center gap-3 rounded-lg p-2 text-left transition-colors hover:bg-white/40 dark:hover:bg-slate-700/40"
            >
              {game.coverImage && (
                <img src={game.coverImage} alt="" className="h-10 w-7 rounded object-cover" />
              )}
            ...
            <div key={game.id} className="border border-editorial-line bg-editorial-2 space-y-3 rounded-xl p-4">
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="shrink-0 rounded-lg p-2 text-gray-400 transition-colors hover:bg-red-500/10 hover:text-red-500"
                >
