...
export function CoachingSessionsContent() {
  ...
  const { data, isLoading, mutate } = useSWR<{ sessions: Session[] }>(
    user ? `/api/coaching/sessions?role=${role}` : null,
    fetcher
  );
  ...
  return (
    <div className="space-y-6 p-4 md:space-y-8 md:p-6 lg:p-8">
      ...
      {paymentMessage && (
        <div
          className={`flex items-center gap-2 rounded-xl p-3 text-sm font-medium ${paymentMessage.type === "success" ? "bg-green-500/10 text-green-500" : "bg-red-500/10 text-red-400"}`}
        >
          <Icon
            icon={paymentMessage.type === "success" ? "lucide:check-circle" : "lucide:alert-circle"}
            className="size-4"
          />
          {paymentMessage.text}
          <button onClick={() => setPaymentMessage(null)} className="ml-auto">
      ...
      <div className="flex gap-2">
        {(["student", "coach"] as const).map((r) => (
          <button
            key={r}
            onClick={() => setRole(r)}
            className={`rounded-lg px-4 py-2 text-sm font-medium transition-all ${role === r ? "from-palette-secondary-500 to-palette-primary-500 bg-linear-to-r text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200 dark:bg-gray-800 dark:text-gray-400"}`}
          >
            {t(`role.${r}`)}
          </button>
        ))}
          ...
          {data?.sessions.map((s) => (
            ...
            >
              ...
              <div className="flex flex-1 items-center justify-between gap-3 sm:justify-end">
                <span
                  className={`flex items-center gap-1 text-xs font-medium ${STATUS_COLORS[s.status]}`}
                >
                  <Icon icon={STATUS_ICONS[s.status]} className="size-4" />{" "}
                  {t(`status.${s.status}`)}
                ...
                <div className="flex gap-1.5">
                  ...
                  {role === "student" && s.status === "confirmed" && s.paymentStatus !== "paid" && (
                    <button
                      ...
                      className="from-palette-secondary-500 to-palette-primary-500 rounded-lg bg-linear-to-r px-3 py-1.5 text-xs font-medium text-white hover:opacity-90"
                    >
                      {t("actions.pay")}
                    </button>
                  )}
