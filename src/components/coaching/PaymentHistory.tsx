...
export function PaymentHistory() {
  ...
  const { data, isLoading } = useSWR<PaymentsResponse>(
    `/api/coaching/payments?page=${page}&limit=${limit}`,
    fetcher
  );
  ...
  const handlePayout = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setPayoutMessage({
          type: "error",
          text: result.error === "No funds available" ? t("noFunds") : t("payoutError"),
        });
      }
    } catch {
  ...
  return (
    <div className="space-y-4">
      ...
      {payoutMessage && (
        <div
          className={`flex items-center gap-2 rounded-xl p-3 text-sm ${payoutMessage.type === "success" ? "bg-green-500/10 text-green-500" : "bg-red-500/10 text-red-400"}`}
        >
          <Icon
            icon={payoutMessage.type === "success" ? "lucide:check-circle" : "lucide:alert-circle"}
            className="size-4"
          />
          {payoutMessage.text}
        </div>
      ...
      {isLoading ? (
        ...
        <>
          {/* Desktop table */}
          <div className="hidden overflow-hidden rounded-xl border border-white/20 md:block dark:border-slate-700/50">
            ...
            {data.payments.map((p) => (
              ...
              >
                ...
                <div className="text-right">
                  ...
                  <p className="text-xs text-gray-400">
                    {p.amount?.toFixed(2)}€ - {p.platformFee?.toFixed(2)}€
                  </p>
                </div>
              </div>
            ))}
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-center gap-2 pt-2">
              <button
                ...
                disabled={page <= 1}
                className="rounded-lg px-3 py-1.5 text-sm text-gray-500 hover:bg-gray-100 disabled:opacity-30 dark:hover:bg-gray-800"
              >
