...
import type { EntityCardConfig, BadgeVariant } from "../EntityCard";
...
const badgeVariantGenerator = fc.constantFrom("metascore" as const, "count" as const, "role" as const);
...
const entityCardConfigGenerator = fc.record({
  ...
  badge: fc.option(fc.record({
    field: fc.constantFrom("metascore" as keyof TestEntity, "gamesCount" as keyof TestEntity, "role" as keyof TestEntity),
    position: badgePositionGenerator,
    variant: badgeVariantGenerator,
  })),
  hoverOverlay: fc.option(fc.record({
    enabled: fc.boolean(),
    showTitle: fc.option(fc.boolean()),
    showDescription: fc.option(fc.boolean()),
    fields: fc.constant([]),
  })),
  actions: fc.option(fc.record({
    libraryToggle: fc.option(fc.boolean()),
    share: fc.option(fc.boolean()),
  })),
  linkTemplate: fc.constant((entity: TestEntity, locale: string) => `/${locale}/test/${entity.slug}`),
});
