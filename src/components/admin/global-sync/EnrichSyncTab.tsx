      ...
      <div className="glass-card rounded-xl p-4 md:p-6">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex gap-2">
            <Button
              onClick={startEnrich}
              disabled={enrichState.isSyncing}
              className="w-full sm:w-auto"
            >
              <Icon
                icon={enrichState.isSyncing ? "svg-spinners:ring-resize" : "lucide:sparkles"}
                className="mr-2 size-4"
              />
              {enrichState.isSyncing ? t("syncing") : t("startSync")}
            </Button>
            {enrichState.isSyncing && (
              <Button variant="outline" onClick={stopEnrich} className="w-full sm:w-auto">
          ...
          <div className="mt-4 space-y-3">
            ...
            {enrichState.currentGame && (
              <p className="text-xs text-gray-500 sm:text-sm dark:text-gray-400">
                {t("currentGame", { name: enrichState.currentGame })}
              </p>
            )}
            <div className="flex flex-wrap gap-3">
              <div className="flex items-center gap-1.5 text-sm">
                ...
                <span className="text-gray-700 dark:text-gray-300">
                  {t("synced", { count: enrichState.totalSynced })}
                </span>
              </div>
              {enrichState.totalFailed > 0 && (
                <div className="flex items-center gap-1.5 text-sm">
                  <Icon icon="lucide:x-circle" className="size-4 text-red-500" />
                  <span className="text-gray-700 dark:text-gray-300">
                    {t("failed", { count: enrichState.totalFailed })}
                  </span>
                </div>
              )}
              {enrichState.remaining > 0 && (
                <div className="flex items-center gap-1.5 text-sm">
                  <Icon icon="lucide:clock" className="size-4 text-gray-400" />
                  <span className="text-gray-700 dark:text-gray-300">
                    {t("remaining", { count: enrichState.remaining })}
                  </span>
                </div>
              )}
            </div>
        ...
        {enrichState.error && <p className="mt-3 text-sm text-red-500">{enrichState.error}</p>}

        {!enrichState.isSyncing && enrichState.totalSynced > 0 && !enrichState.error && (
          <p className="mt-3 text-sm text-green-600 dark:text-green-400">
            {t("complete", { count: enrichState.totalSynced })}
          </p>
        )}
      </div>
