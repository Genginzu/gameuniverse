  ...
  return (
    <div className="space-y-4 md:space-y-6">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex gap-2">
            <Button onClick={p.onStart} disabled={s.isSyncing} className="w-full sm:w-auto">
              <Icon
                icon={s.isSyncing ? "svg-spinners:ring-resize" : p.startIcon}
          ...
          <div className="mt-4 space-y-3">
            ...
            {s.currentGame && (
              <p className="text-xs text-gray-500 sm:text-sm dark:text-gray-400">
                {t("currentGame", { name: s.currentGame })}
              </p>
            )}
            <div className="flex flex-wrap gap-3">
              <StatBadge
                ...
                label={t("synced", { count: s.totalSynced })}
              />
              {p.renderExtraStats?.()}
              {s.totalFailed > 0 && (
                <StatBadge
                  ...
                  color="text-red-500"
                  label={t("failed", { count: s.totalFailed })}
                />
              )}
              {s.remaining > 0 && (
                <StatBadge
                  icon="lucide:clock"
                  color="text-gray-400"
                  label={t("remaining", { count: s.remaining })}
                />
              )}
            </div>
          </div>
      ...
      <div className="flex gap-2">
        ...
        />
        <Button variant="outline" onClick={p.onSearch} className="shrink-0">
          <Icon icon="fa:search" className="size-4" />
        </Button>
      </div>

      <p className="text-sm text-gray-500 dark:text-gray-400">
        {t("remaining", { count: p.total })}
      </p>
            ...
            >
              <div className="size-10 shrink-0 overflow-hidden rounded-lg bg-gray-200 md:size-12 dark:bg-gray-700">
                {cover(entry.cover_image_id) ? (
                  <img
                    src={cover(entry.cover_image_id)!}
                    alt={entry.name}
