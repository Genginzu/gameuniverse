      ...
      <div className="glass-card rounded-xl p-4 md:p-6">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex gap-2">
            <Button
              onClick={startColors}
              disabled={colorsState.isSyncing}
              className="w-full sm:w-auto"
            >
              <Icon
                icon={colorsState.isSyncing ? "svg-spinners:ring-resize" : "lucide:palette"}
                className="mr-2 size-4"
              />
              {colorsState.isSyncing ? t("syncing") : t("startSync")}
            </Button>
            {colorsState.isSyncing && (
              <Button variant="outline" onClick={stopColors} className="w-full sm:w-auto">
        ...
        {(colorsState.isSyncing || colorsState.totalSynced > 0) && (
          <div className="mt-4 space-y-3">
            {colorsState.isSyncing && (
              <SyncProgressBar
                synced={colorsState.totalSynced}
                failed={colorsState.totalFailed}
                remaining={colorsState.remaining}
              />
            )}
            {colorsState.currentGame && (
              <p className="text-xs text-gray-500 sm:text-sm dark:text-gray-400">
                {t("currentGame", { name: colorsState.currentGame })}
              </p>
            )}
            <div className="flex flex-wrap gap-3">
              <div className="flex items-center gap-1.5 text-sm">
                ...
                <span className="text-gray-700 dark:text-gray-300">
                  {t("synced", { count: colorsState.totalSynced })}
                </span>
              </div>
              {colorsState.totalFailed > 0 && (
                <div className="flex items-center gap-1.5 text-sm">
                  <Icon icon="lucide:x-circle" className="size-4 text-red-500" />
                  <span className="text-gray-700 dark:text-gray-300">
                    {t("failed", { count: colorsState.totalFailed })}
                  </span>
                </div>
              )}
              {colorsState.remaining > 0 && (
                <div className="flex items-center gap-1.5 text-sm">
                  <Icon icon="lucide:clock" className="size-4 text-gray-400" />
                  <span className="text-gray-700 dark:text-gray-300">
                    {t("remaining", { count: colorsState.remaining })}
                  </span>
                </div>
              )}
            </div>
        ...
        {colorsState.error && <p className="mt-3 text-sm text-red-500">{colorsState.error}</p>}
      </div>
