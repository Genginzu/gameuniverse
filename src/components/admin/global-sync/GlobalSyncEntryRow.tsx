...
export function GlobalSyncEntryRow({ entry, t }: { entry: GlobalSyncEntry; t: (key: string) => string }) {
  const coverUrl = entry.cover_image_id ? `${IGDB_IMAGE_BASE}/t_cover_small/${entry.cover_image_id}.jpg` : null;
  ...
  return (
    <div className="glass-card flex items-center gap-3 rounded-xl p-3 md:gap-4 md:p-4">
      ...
      <Badge variant={entry.matched_game_id ? "default" : "secondary"} className={entry.matched_game_id ? "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400" : ""}>
