...
import { EDITABLE_FIELDS, type TranslationMissingItem, type EntityType } from "@/types/admin-translations";
...
export function TranslationReviewModal({ isOpen, onClose, item, translatedFields, entityType, targetLang, onSave, isSaving }: TranslationReviewModalProps) {
  ...
  useEffect(() => { reset(translatedFields); }, [translatedFields, reset]);

  if (!isOpen) return null;

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" onClick={onClose}>
      <div className="glass-card mx-4 w-full max-w-3xl rounded-2xl border border-white/20 p-6 shadow-xl dark:border-slate-700/50" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="mb-4 text-xs text-gray-500 dark:text-gray-400">{item.identifier} — {t(`entityTypes.${entityType}`)} — {t(`languages.${targetLang}`)}</div>
        <form onSubmit={handleSubmit(async (data) => { await onSave(data); })} className="space-y-4">
          {fields.map((field) => (
            <TranslationFieldRow key={field} field={field} sourceValue={item.sourceText[field] ?? ""} register={register} error={!!errors[field]} />
          ))}
