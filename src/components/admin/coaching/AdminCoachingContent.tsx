...
interface Coach {
  id: string; username: string; avatarUrl: string | null; email: string;
  isActive: boolean; isVerified: boolean; isSuspended: boolean;
  averageRating: number; totalReviews: number; totalSessions: number;
  stripeComplete: boolean; createdAt: string;
}
...
export function AdminCoachingContent() {
  ...
  const { data, isLoading, mutate } = useSWR<{ coaches: Coach[]; total: number }>(
    `/api/admin/coaching?status=${filter}`, fetcher
  );
  ...
  return (
    <div className="space-y-6 p-4 md:p-6 lg:p-8">
      <div className="flex items-center justify-between">
        ...
        <span className="text-sm text-gray-500">{data?.total ?? 0} {t("coaches")}</span>
      </div>
      ...
      {isLoading ? (
        <div className="space-y-3">{Array.from({ length: 3 }).map((_, i) => <div key={i} className="h-20 animate-pulse rounded-xl bg-gray-200 dark:bg-gray-700" />)}</div>
      ) : (
          ...
          {data?.coaches.map((c) => (
            <div key={c.id} className="glass-card flex flex-col gap-3 rounded-xl p-4 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex gap-1.5">
                {!c.isVerified ? (
                  <button onClick={() => handleAction(c.id, "verify")} className="rounded-lg bg-green-500/10 px-3 py-1.5 text-xs font-medium text-green-500 hover:bg-green-500/20">
                    <Icon icon="lucide:check" className="mr-1 inline size-3.5" />{t("actions.verify")}
                  </button>
                ) : (
                  <button onClick={() => handleAction(c.id, "unverify")} className="rounded-lg bg-gray-100 px-3 py-1.5 text-xs font-medium text-gray-500 hover:bg-gray-200 dark:bg-gray-800">
                    {t("actions.unverify")}
                  </button>
                )}
                {!c.isSuspended ? (
                  <button onClick={() => handleAction(c.id, "suspend", "Admin decision")} className="rounded-lg bg-red-500/10 px-3 py-1.5 text-xs font-medium text-red-400 hover:bg-red-500/20">
