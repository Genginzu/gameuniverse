...
export function AdminReviewsTable({ reviews, pagination, onPageChange, onSearch, onSort, onEdit, onDelete, canDelete, isLoading, currentSort, currentSearch = "" }: AdminReviewsTableProps) {
  ...
  const formatDate = (dateStr: string) => { try { return new Date(dateStr).toLocaleDateString(); } catch { return "—"; } };
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-sm text-gray-500 dark:text-gray-400">{t("totalReviews", { count: pagination.totalCount })}</p>

      {isLoading ? <AdminTableSkeleton columns={5} rows={8} /> : reviews.length === 0 ? (
        <div className="rounded-xl border border-gray-200 bg-white p-12 text-center dark:border-gray-700 dark:bg-gray-800"><p className="text-gray-500 dark:text-gray-400">{t("noReviews")}</p></div>
      ) : (
        <>
          <div className="overflow-x-auto rounded-xl border border-gray-200 dark:border-gray-700">
            <table className="w-full text-left text-sm" role="table">
              <thead className="border-b border-gray-200 bg-gray-50 dark:border-gray-700 dark:bg-gray-800">
                <tr>
                  {sortHeaders.map((h) => (
                    <th key={h.field} scope="col" className="px-4 py-3"><button type="button" className="inline-flex items-center gap-1 font-medium text-gray-600 hover:text-gray-900 dark:text-gray-300 dark:hover:text-white" onClick={() => handleSortClick(h.field)}>{h.label}{renderSortIcon(h.field)}</button></th>
                  ))}
                  <th scope="col" className="px-4 py-3 font-medium text-gray-600 dark:text-gray-300">{t("columns.content")}</th>
                  <th scope="col" className="px-4 py-3"><button type="button" className="inline-flex items-center gap-1 font-medium text-gray-600 hover:text-gray-900 dark:text-gray-300 dark:hover:text-white" onClick={() => handleSortClick("created_at")}>{t("columns.date")}{renderSortIcon("created_at")}</button></th>
