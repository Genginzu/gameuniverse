...
import { CHARACTER_TRACKABLE_FIELDS } from "@/lib/services/igdb-character-sync";
import type { CharacterTrackableField } from "@/types/admin-characters";
import { Icon } from "@iconify/react";

interface CharacterFormSyncTabProps {
  characterId: string;
  igdbId: number | null;
  /** Called after a successful sync so the parent can reload character data */
  onSyncComplete?: () => void;
}

/** Maps each trackable field to its i18n label key */
const FIELD_LABEL_KEYS: Record<CharacterTrackableField, string> = {
...
export function CharacterFormSyncTab({
  characterId,
  igdbId,
  onSyncComplete,
}: CharacterFormSyncTabProps) {
  const t = useTranslations("admin.characters.sync");
    ...
    <div className="space-y-4">
      ...
      {loadingOverrides ? (
        ...
        <div className="divide-y divide-gray-100 rounded-lg border border-gray-200 dark:divide-gray-700 dark:border-gray-700">
          {CHARACTER_TRACKABLE_FIELDS.map((field) => {
