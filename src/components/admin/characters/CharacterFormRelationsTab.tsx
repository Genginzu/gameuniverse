...
export function CharacterFormRelationsTab({
  ...
  const addRelation = (characterId: string) => {
    ...
    form.setValue("relationships", [...current, { related_character_id: characterId, relationship_type: "ally", description: "" }], { shouldValidate: true });
    setShowPicker(false);
  };

  const removeRelation = (characterId: string) => {
    const current = form.getValues("relationships");
    form.setValue("relationships", current.filter((r) => r.related_character_id !== characterId), { shouldValidate: true });
  };

  const updateRelationType = (characterId: string, type: string) => {
    const current = form.getValues("relationships");
    form.setValue("relationships", current.map((r) => r.related_character_id === characterId ? { ...r, relationship_type: type as typeof r.relationship_type } : r), { shouldValidate: true });
  };

  const updateRelationDescription = (characterId: string, description: string) => {
    const current = form.getValues("relationships");
    form.setValue("relationships", current.map((r) => (r.related_character_id === characterId ? { ...r, description } : r)), { shouldValidate: true });
  };

  return (
    <div className="space-y-4">
      {watchedRelations.length === 0 ? (
        <p className="py-6 text-center text-sm text-gray-400 dark:text-gray-500">{t("noRelations") ?? "Aucune relation ajoutée."}</p>
      ) : (
          ...
          {watchedRelations.map((rel) => {
            ...
            return (
              <RelationRow
                ...
                onChangeDescription={(desc) => updateRelationDescription(rel.related_character_id, desc)}
      ...
      {showPicker ? (
        <CharacterSearchPicker availableCharacters={pickableCharacters} onSelect={addRelation} onClose={() => setShowPicker(false)} t={t} />
      ) : (
        <Button type="button" variant="outline" size="sm" onClick={() => setShowPicker(true)} className="gap-1.5" disabled={pickableCharacters.length === 0}>
