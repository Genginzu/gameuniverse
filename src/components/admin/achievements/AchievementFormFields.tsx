...
import { AchievementTranslationsSection, AchievementNumericSection } from "./AchievementFormSections";
        ...
        <div className="grid gap-5 sm:grid-cols-2">
          <FormField control={form.control} name="key" render={({ field }) => (
            <FormItem>
              ...
              <FormControl><Input placeholder={t("keyPlaceholder")} {...field} disabled={mode === "edit"} className={mode === "edit" ? "bg-gray-50 dark:bg-gray-900/50" : ""} maxLength={100} /></FormControl>
              <FormDescription>{t("keyDescription")}</FormDescription>
              <FormMessage />
            </FormItem>
          )} />
          <FormField control={form.control} name="icon" render={({ field }) => (
            <FormItem><FormLabel>{t("icon")}</FormLabel><FormControl><Input placeholder={t("iconPlaceholder")} {...field} maxLength={50} /></FormControl><FormMessage /></FormItem>
          )} />
          <FormField control={form.control} name="category" render={({ field }) => (
            <FormItem>
              ...
              <Select onValueChange={field.onChange} defaultValue={field.value}>
                ...
                <SelectContent>{CATEGORIES.map((cat) => <SelectItem key={cat} value={cat}>{tCat(cat)}</SelectItem>)}</SelectContent>
              </Select>
              <FormMessage />
            </FormItem>
          )} />
          <FormField control={form.control} name="tier" render={({ field }) => (
            <FormItem>
              ...
              <Select onValueChange={field.onChange} defaultValue={field.value}>
                <FormControl><SelectTrigger><SelectValue placeholder={t("tierPlaceholder")} /></SelectTrigger></FormControl>
                <SelectContent>{TIERS.map((tier) => <SelectItem key={tier} value={tier}>{tTier(tier)}</SelectItem>)}</SelectContent>
              </Select>
              <FormMessage />
            </FormItem>
          )} />
