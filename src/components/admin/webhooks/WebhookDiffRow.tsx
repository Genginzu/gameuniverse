...
export function DiffRow({ field, forced, onToggleForce, applied }: DiffRowProps) {
  ...
  return (
    <tr
      className={`transition-colors ${isChanged ? "bg-yellow-50/30 dark:bg-yellow-900/5" : "hover:bg-gray-50/50 dark:hover:bg-gray-800/30"}`}
    >
      ...
      <td
        className={`px-4 py-3 ${isChanged ? "text-palette-secondary-700 dark:text-palette-secondary-400 font-medium" : "text-gray-600 dark:text-gray-300"}`}
      >
        <div className="break-words whitespace-pre-wrap">{formatValue(field.igdbValue)}</div>
      </td>
      <td className="px-4 py-3">
        <span
          className={`inline-flex rounded-full px-2 py-0.5 text-xs font-medium ${statusStyles[field.status]}`}
        >
          {t(`statuses.${field.status}`)}
        </span>
      </td>
      <td className="px-4 py-3">
        {field.status === "conflict" && !applied && (
          <button
            ...
            className={`flex items-center gap-1 rounded-lg px-3 py-1.5 text-xs font-medium transition-all ${forced ? "bg-amber-500 text-white shadow-sm" : "border border-amber-300 text-amber-700 hover:bg-amber-50 dark:border-amber-600 dark:text-amber-400 dark:hover:bg-amber-900/20"}`}
          >
