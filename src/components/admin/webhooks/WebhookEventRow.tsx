    ...
    <tr className="transition-colors hover:bg-gray-50/50 dark:hover:bg-gray-800/30">
      <td className="px-4 py-3">
        <div className="flex items-center gap-2">
          <Icon icon={EVENT_ICONS[event.event_type] ?? "lucide:circle"} className={`h-4 w-4 ${EVENT_COLORS[event.event_type] ?? "text-gray-500"}`} />
          <span className={`font-medium ${EVENT_COLORS[event.event_type] ?? "text-gray-900 dark:text-white"}`}>{t(`eventTypes.${event.event_type}`)}</span>
        </div>
      </td>
      <td className="px-4 py-3">
        {entityLink ? (
      ...
      <td className="px-4 py-3 font-mono text-xs text-gray-500 dark:text-gray-400">{event.igdb_id}</td>
      <td className="px-4 py-3">
        <span className={`inline-flex rounded-full px-2 py-0.5 text-xs font-medium ${STATUS_STYLES[event.status] ?? ""}`}>{t(`statuses.${event.status}`)}</span>
        {isCreateSuccess && (
          <span className="ml-2 inline-flex items-center gap-1 rounded-full bg-green-100 px-2 py-0.5 text-xs font-medium text-green-700 dark:bg-green-900/30 dark:text-green-400">
            <Icon icon="lucide:check-circle" className="h-3 w-3" />{t("importSuccess")}
          </span>
        )}
        {event.error_message && <p className="mt-1 max-w-xs truncate text-xs text-red-500" title={event.error_message}>{event.error_message}</p>}
      </td>
      <td className="px-4 py-3 text-xs text-gray-500 dark:text-gray-400">
        <div className="flex items-center gap-2">
          <span>{formattedDate}</span>
          {isCreateSuccess && entityLink && (
            <Link href={entityLink} className="inline-flex items-center gap-1 rounded-lg bg-green-50 px-2 py-1 text-xs font-medium text-green-700 transition-colors hover:bg-green-100 dark:bg-green-900/20 dark:text-green-400 dark:hover:bg-green-900/40">
              <Icon icon="lucide:external-link" className="h-3 w-3" />{t("viewGame")}
            </Link>
          )}
          {diffLink && (
          ...
          {canDelete && event.game_id && (
            <button type="button" disabled={isDeleting} onClick={() => onDeleteGame?.(event.game_id!)} className="inline-flex items-center gap-1 rounded-lg bg-red-50 px-2 py-1 text-xs font-medium text-red-700 transition-colors hover:bg-red-100 disabled:opacity-50 dark:bg-red-900/20 dark:text-red-400 dark:hover:bg-red-900/40">
              {isDeleting ? <Icon icon="lucide:loader-2" className="h-3 w-3 animate-spin" /> : <Icon icon="lucide:trash-2" className="h-3 w-3" />}
