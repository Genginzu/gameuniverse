...
export function PlatformList({ platforms, pagination, onPageChange, onSearch, onSort, onEdit, onDelete, isLoading, currentSort, currentSearch = "" }: PlatformListProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-sm text-gray-500 dark:text-gray-400">{t("totalPlatforms", { count: pagination.totalCount })}</p>

      {isLoading ? <AdminTableSkeleton columns={3} rows={6} /> : platforms.length === 0 ? (
        <div className="rounded-xl border border-white/20 bg-white/40 p-12 text-center backdrop-blur-xl dark:border-slate-700/50 dark:bg-slate-800/50"><p className="text-gray-500 dark:text-gray-400">{t("noPlatforms")}</p></div>
      ) : (
        <>
          <div className="overflow-x-auto rounded-xl border border-white/20 dark:border-slate-700/50">
            <table className="w-full text-left text-sm" role="table">
              <thead className="border-b border-white/20 bg-white/40 backdrop-blur-xl dark:border-slate-700/50 dark:bg-slate-800/50">
                <tr>
                  <th scope="col" className="px-4 py-3"><button type="button" className="inline-flex items-center gap-1 font-medium text-gray-600 hover:text-gray-900 dark:text-gray-300 dark:hover:text-white" onClick={() => handleSortClick("slug")}>{t("columns.slug")}{renderSortIcon("slug")}</button></th>
                  ...
                  <th scope="col" className="px-4 py-3 font-medium text-gray-600 dark:text-gray-300">{t("columns.icon")}</th>
                  <th scope="col" className="px-4 py-3"><button type="button" className="inline-flex items-center gap-1 font-medium text-gray-600 hover:text-gray-900 dark:text-gray-300 dark:hover:text-white" onClick={() => handleSortClick("game_count")}>{t("columns.gameCount")}{renderSortIcon("game_count")}</button></th>
                    ...
                    <tr key={platform.id} className="cursor-pointer transition-all hover:bg-white/60 dark:hover:bg-slate-700/60" onClick={() => onEdit(platform)}>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
                          <Button variant="ghost" size="sm" onClick={() => onEdit(platform)} aria-label={t("editPlatform", { name })} className="min-h-[44px] min-w-[44px]"><Icon icon="fa:edit" className="h-4 w-4" /></Button>
                          <Button variant="ghost" size="sm" onClick={() => onDelete(platform)} aria-label={t("deletePlatform", { name })} className="min-h-[44px] min-w-[44px] text-red-600 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300"><Icon icon="fa:trash" className="h-4 w-4" /></Button>
                        </div>
                      </td>
                    </tr>
                  );
