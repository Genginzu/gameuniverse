 ...
 *
 * This test validates the locale-based content display logic used in:
 * - CharacterCard component
 * - CharacterDetailsContent component
 * - AllCharactersContent component
 ...
 * This mirrors the actual behavior in:
 * - src/app/api/characters/route.ts (filters by language_code)
 * - Components that use useTranslations() hook
 */
 ...
 * This mirrors the useTranslations() hook behavior.
 */
...
/**
 * Validates that a string appears to be in the expected language.
 * Uses simple heuristics based on common French/English patterns.
 */
const containsFrenchPatterns = (text: string): boolean => {
  // Common French patterns: accented characters, French words
  const frenchPatterns =
    /[éèêëàâäùûüôöîïç]|(\ble\b|\bla\b|\bles\b|\bde\b|\bdu\b|\bdes\b|\bun\b|\bune\b)/i;
  return frenchPatterns.test(text);
};

const containsEnglishPatterns = (text: string): boolean => {
  // Common English patterns: the, a, an, of, in
  const englishPatterns = /(\bthe\b|\ba\b|\ban\b|\bof\b|\bin\b|\bis\b|\bare\b)/i;
...
describe("Character Pages Property-Based Tests - Locale Content Display", () => {
    ...
    it("French locale returns French character content", () => {
      fc.assert(
        fc.property(characterTranslationsGenerator, (character) => {
          ...
          // The localized character should have French content
    ...
    it("English locale returns English character content", () => {
      fc.assert(
        fc.property(characterTranslationsGenerator, (character) => {
          ...
          // The localized character should have English content
    ...
    it("locale selection is deterministic - same locale always returns same content", () => {
      fc.assert(
        fc.property(characterTranslationsGenerator, localeGenerator, (character, locale) => {
          ...
          // Same locale should always return identical content
      ...
      fc.assert(
        fc.property(
          ...
          (character) => {
            ...
            // Different locales should return different names
    ...
    it("character ID and slug are preserved regardless of locale", () => {
      fc.assert(
        fc.property(characterTranslationsGenerator, localeGenerator, (character, locale) => {
          ...
          // ID and slug should be the same regardless of locale
    ...
    it("role translations are consistent with locale", () => {
      ...
      fc.assert(
        fc.property(localeGenerator, fc.constantFrom(...roleKeys), (locale, roleKey) => {
          ...
          // Role translation should exist and be non-empty
          expect(roleTranslation).toBeDefined();
          expect(roleTranslation.length).toBeGreaterThan(0);

          // French roles should be different from English roles
    ...
    it("all character fields are localized together", () => {
      fc.assert(
        fc.property(characterTranslationsGenerator, localeGenerator, (character, locale) => {
          ...
          // All fields should come from the same locale
    ...
    it("optional fields are correctly handled for both locales", () => {
      fc.assert(
        fc.property(characterTranslationsGenerator, localeGenerator, (character, locale) => {
          ...
          // Optional fields should be undefined when not present in translation
      ...
      fc.assert(
        fc.property(
          ...
          (characters, locale) => {
            ...
            // All characters should have content from the same locale
  ...
  describe("Property 12 Extended: API Locale Parameter Handling", () => {
    /**
     * Simulates the API's locale parameter handling.
     * This mirrors the behavior in src/app/api/characters/route.ts
     */
  ...
  describe("Property 12 Extended: Link Generation with Locale", () => {
    /**
     * Simulates the link generation logic used in CharacterCard.
     * Links should include the correct locale prefix.
     */
