...
export function CharacterFilters({
  ...
  const hasFilters = selectedGames.length > 0 || selectedRoles.length > 0;
        ...
        <div className="rounded-xl bg-gradient-to-r from-blue-50 to-indigo-50 p-3 sm:p-4">
          <div className="mb-2 flex items-center justify-between">
            ...
            <span className="text-xs text-blue-600">
              {selectedGames.length + selectedRoles.length} sélectionné(s)
            </span>
          </div>
          <div className="flex flex-wrap gap-2">
            {selectedGames.map((gameId) => {
              ...
              return (
                ...
                >
                  <span className="mr-1 sm:mr-2">{game?.title || gameId}</span>
                  <button
            ...
            {selectedRoles.map((roleValue) => {
              ...
              return (
                <div
                  ...
                  className="inline-flex items-center rounded-lg bg-white px-2 py-1 text-xs font-medium text-gray-700 shadow-sm ring-1 ring-gray-200 sm:px-3 sm:py-1.5 sm:text-sm"
                >
                  <span className="mr-1 sm:mr-2">{role?.label_fr || roleValue}</span>
                  <button
        ...
        <div className="space-y-6 rounded-xl bg-white p-4 shadow-lg ring-1 ring-gray-200 sm:p-6">
          ...
          <div>
            <div className="mb-4 flex flex-col items-start justify-between sm:flex-row sm:items-center">
              <h3 className="text-base font-semibold text-gray-900 sm:text-lg">Filtrer par jeu</h3>
              <span className="mt-1 text-sm text-gray-500 sm:mt-0">{games.length} disponibles</span>
            </div>
                ...
                >
                  ...
                  <div className="ml-2 min-w-0 flex-1 sm:ml-3">
                    ...
                    {game.characterCount !== undefined && (
                      <div className="text-xs text-gray-500">{game.characterCount} personnages</div>
                    )}
                  </div>
                </label>
              ))}
          ...
          <div>
            <div className="mb-4 flex flex-col items-start justify-between sm:flex-row sm:items-center">
              <h3 className="text-base font-semibold text-gray-900 sm:text-lg">Filtrer par rôle</h3>
