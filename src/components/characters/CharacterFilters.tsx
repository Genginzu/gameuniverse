...
import { CharacterPlatformFilter } from "./CharacterPlatformFilter";
...
/** Reusable badge for active filter display */
function FilterBadge({ label, onRemove }: { label: string; onRemove: () => void }) {
  return (
    <div className="inline-flex items-center rounded-lg bg-white px-2 py-1 text-xs font-medium text-gray-700 shadow-sm ring-1 ring-gray-200 dark:bg-gray-800 dark:text-gray-200 dark:ring-gray-700 sm:px-3 sm:py-1.5 sm:text-sm">
      <span className="mr-1 sm:mr-2">{label}</span>
      <button
        onClick={onRemove}
...
export function CharacterFilters({
  ...
  return (
    <div className="space-y-4">
      ...
      {hasFilters && (
        <div className="flex justify-end">
          <button
            ...
            className="inline-flex items-center rounded-lg bg-red-50 px-3 py-2 text-sm font-medium text-red-700 transition-colors hover:bg-red-100 dark:bg-red-900/20 dark:text-red-400 dark:hover:bg-red-900/30 sm:px-4"
          >
      ...
      {hasFilters && (
        <div className="rounded-xl bg-gradient-to-r from-blue-50 to-indigo-50 p-3 dark:from-blue-900/20 dark:to-indigo-900/20 sm:p-4">
          <div className="mb-2 flex items-center justify-between">
            <span className="text-sm font-medium text-blue-900 dark:text-blue-300">
              Filtres actifs
            </span>
            <span className="text-xs text-blue-600 dark:text-blue-400">
              {selectedGames.length + selectedRoles.length + selectedPlatforms.length}{" "}
              sélectionné(s)
            </span>
          </div>
          <div className="flex flex-wrap gap-2">
            {selectedGames.map((gameId) => {
              ...
              return (
                <FilterBadge
                  ...
                  onRemove={() => handleGameToggle(gameId)}
                />
              );
            })}
            {selectedRoles.map((roleValue) => {
              ...
              return (
                <FilterBadge
                  key={roleValue}
                  label={role?.label_fr || roleValue}
                  onRemove={() => handleRoleToggle(roleValue)}
                />
              );
            })}
      ...
      {/* Expanded filters */}
      {showAllFilters && (
        <div className="space-y-6 rounded-xl bg-white p-4 shadow-lg ring-1 ring-gray-200 dark:bg-gray-800 dark:ring-gray-700 sm:p-6">
              ...
              {games.map((game) => (
                ...
                >
                  ...
                  <div className="ml-2 min-w-0 flex-1 sm:ml-3">
                    ...
                    {game.characterCount !== undefined && (
                      <div className="text-xs text-gray-500 dark:text-gray-400">
                        {game.characterCount} personnages
                      </div>
                    )}
                  </div>
                </label>
          ...
          <div>
            <div className="mb-4 flex flex-col items-start justify-between sm:flex-row sm:items-center">
              ...
              <span className="mt-1 text-sm text-gray-500 dark:text-gray-400 sm:mt-0">
                {AVAILABLE_ROLES.length} disponibles
              </span>
            </div>
