    ...
    <section className="mb-8 space-y-4">
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        {isOwner && (
          <button
            ...
            className="from-palette-primary-500 shadow-palette-primary-500/20 inline-flex shrink-0 items-center gap-2 rounded-2xl bg-linear-to-br to-blue-500 px-5 py-3 text-sm font-medium text-white shadow-lg transition-all duration-300 hover:opacity-90"
          >
            {showComposer ? (
      ...
      <div role="feed" aria-busy={isLoading || isLoadingMore} aria-label={t("feedLabel")}>
        ...
        {!isLoading && posts.length > 0 && (
          <div className="border-palette-primary-300/50 dark:border-palette-primary-500/30 relative ml-6 border-l-2 pl-6">
            {posts.map((post) => (
              <div key={post.id} className="relative mb-6">
                {/* Timeline dot */}
                <div className="border-palette-primary-400 dark:border-palette-primary-500 absolute top-5 -left-[33px] h-4 w-4 rounded-full border-2 bg-white dark:bg-slate-800" />
                <PostCard
                  post={post}
