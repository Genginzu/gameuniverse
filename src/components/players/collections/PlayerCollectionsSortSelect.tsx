  ...
  return (
    <Select value={value} onValueChange={(v) => onChange(v as CollectionSortOption)}>
      <SelectTrigger
        ...
        className="glass-dropdown focus:ring-neon-primary/30 dark:focus:ring-neon-primary/40 w-auto cursor-pointer gap-2 rounded-xl border-0 px-4 py-2 text-sm font-medium text-gray-700 transition-all duration-300 focus:ring-2 focus:ring-offset-0 dark:text-slate-200"
      >
        <SelectValue />
      </SelectTrigger>
      <SelectContent className="glass-dropdown z-50 min-w-40 overflow-hidden rounded-xl border-0 p-1">
        {SORT_OPTIONS.map((option) => (
          <SelectItem
            key={option}
            value={option}
            className="data-[state=checked]:bg-neon-primary/10 data-[state=checked]:text-neon-primary dark:data-[state=checked]:bg-neon-primary/20 dark:data-[state=checked]:text-palette-primary-300 cursor-pointer rounded-lg py-2 pr-3 pl-3 text-sm font-medium text-gray-700 transition-colors hover:bg-white/60 focus:bg-white/60 dark:text-slate-200 dark:hover:bg-slate-700/60 dark:focus:bg-slate-700/60 [&>span:first-child]:hidden"
          >
            {t(SORT_KEYS[option])}
          </SelectItem>
        ))}
