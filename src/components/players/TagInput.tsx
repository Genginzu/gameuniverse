...
export function TagInput({ tags, onChange }: TagInputProps) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5 rounded-xl border-2 border-violet-300 bg-white/60 px-3 py-2 shadow-sm backdrop-blur-sm transition-all duration-200 focus-within:border-violet-400 focus-within:ring-2 focus-within:ring-violet-400/20 dark:border-violet-500/50 dark:bg-slate-700/40 dark:focus-within:border-violet-400/60 dark:focus-within:ring-violet-400/15">
      ...
      {tags.length < MAX_TAGS && (
        <input
          ...
          className="min-w-[80px] flex-1 border-none bg-transparent text-xs text-gray-800 placeholder-gray-400 outline-none ring-0 focus:border-none focus:outline-none focus:ring-0 dark:text-slate-100 dark:placeholder-slate-500"
        />
      )}
    </div>
  );
