...
import { useBulkImport, type BulkImportField } from "@/hooks/useBulkImport";
import { BulkImportGameList } from "@/components/admin/bulk-import/BulkImportGameList";

const VALID_FIELDS: BulkImportField[] = [
  "cover",
  "background",
  "playtime",
  "metascore",
...
export default function BulkImportFieldPage() {
  ...
  const isValid = VALID_FIELDS.includes(field as BulkImportField);
  ...
  const {
    ...
    handleSync,
    handleAbort,
  } = useBulkImport(isValid ? (field as BulkImportField) : "cover");
  ...
  if (!isValid) {
    return (
      <div className="flex flex-col items-center gap-4 p-8">
        ...
        <Link href="/admin/bulk-import" className="text-palette-secondary-500 hover:underline">
          {t("backToFields")}
        </Link>
      </div>
    );
  ...
  return (
    <div className="space-y-6 p-4 lg:p-6">
      ...
      <BulkImportGameList
        ...
        syncing={syncing}
        batchSize={batchSize}
        selectedField={field as BulkImportField}
        progress={progress}
