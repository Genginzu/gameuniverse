...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data, error } = await supabase
      ...
      .select("id, rating, comment, coach_response, coach_responded_at, created_at, student_id, profiles!coaching_reviews_student_id_fkey(username, avatar_url)")
      .eq("coach_id", coachId)
    ...
    const reviews = (data || []).map((r: S) => ({
      ...
      coachResponse: r.coach_response, coachRespondedAt: r.coach_responded_at,
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: session } = await supabase
      .from("coaching_sessions").select("id, coach_id, student_id, status")
      .eq("id", sessionId).single();

    if (!session) return NextResponse.json({ error: "Session not found" }, { status: 404 });
    if (session.student_id !== user.id) return NextResponse.json({ error: "Forbidden" }, { status: 403 });
    if (session.status !== "completed") return NextResponse.json({ error: "Session must be completed" }, { status: 400 });

    // Check no existing review
    const { data: existing } = await supabase
      .from("coaching_reviews").select("id").eq("session_id", sessionId).single();
    if (existing) return NextResponse.json({ error: "Already reviewed" }, { status: 409 });

    const { data: review, error } = await supabase
      .from("coaching_reviews")
      .insert({ session_id: sessionId, coach_id: session.coach_id, student_id: user.id, rating, comment: comment || null })
