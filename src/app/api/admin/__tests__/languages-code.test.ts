...
} = await import("@/app/api/admin/languages/[code]/route");
...
describe("Admin Languages [code] API", () => {
  ...
  describe("GET /api/admin/languages/[code]", () => {
    ...
    test("returns language when found", async () => {
      const singleMock = vi.fn(() => Promise.resolve({ data: sampleLanguage, error: null }));
      ...
      mockSupabaseFrom = vi.fn(() => {
        ...
        if (callCount === 1) {
          ...
          return {
            select: vi.fn(() => ({
              eq: vi.fn(() => ({
                single: vi.fn(() => Promise.resolve({ data: { code: "fr" }, error: null })),
              })),
            })),
          };
        }
        ...
        return {
          update: vi.fn(() => ({
            eq: vi.fn(() => ({
              select: vi.fn(() => ({
                single: vi.fn(() => Promise.resolve({ data: updatedLang, error: null })),
              })),
            })),
          })),
        };
      ...
      mockSupabaseFrom = vi.fn((table: string) => {
        ...
        if (table === "supported_languages") {
          ...
          return {
            select: vi.fn(() => ({
              eq: vi.fn(() => ({
                single: vi.fn(() => Promise.resolve({ data: sampleLanguage, error: null })),
              })),
            })),
          };
        }
