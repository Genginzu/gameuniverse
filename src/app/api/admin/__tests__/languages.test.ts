  ...
  return {
    select: vi.fn((_cols: string, _opts?: unknown) => {
      const chain = {
        or: vi.fn(() => chain),
        order: vi.fn(() => chain),
        range: vi.fn(() => Promise.resolve({ data: merged.data, error: merged.error })),
        // For count queries (head: true)
        ...((_opts as { head?: boolean })?.head
          ? { or: vi.fn(() => Promise.resolve({ count: merged.count, error: merged.error })) }
          : {}),
      };
...
// Import after mocks
const { GET, POST } = await import("@/app/api/admin/languages/route");
...
describe("Admin Languages API", () => {
  ...
  describe("GET /api/admin/languages", () => {
    ...
    test("returns languages with pagination structure", async () => {
      const selectMock = vi.fn((_cols: string, opts?: { count?: string; head?: boolean }) => {
        ...
        return {
          or: vi.fn(() => ({
            order: vi.fn(() => ({
              range: vi.fn(() => Promise.resolve({ data: mockLanguagesData, error: null })),
            })),
          })),
          order: vi.fn(() => ({
            range: vi.fn(() => Promise.resolve({ data: mockLanguagesData, error: null })),
    ...
    test("applies search filter via or clause", async () => {
      const orMock = vi.fn((_filter: string) => ({
        order: vi.fn(() => ({
          range: vi.fn(() =>
            Promise.resolve({
  ...
  describe("POST /api/admin/languages", () => {
    ...
    test("creates language successfully with valid data", async () => {
      ...
      const singleMock = vi.fn(() => Promise.resolve({ data: newLang, error: null }));
      const selectMock = vi.fn(() => ({ single: singleMock }));
      const insertMock = vi.fn(() => ({ select: selectMock }));

      mockSupabaseFrom = vi.fn(() => ({ insert: insertMock }));
    ...
    test("creates language with empty native_name as empty string (NOT NULL column)", async () => {
      ...
      const singleMock = vi.fn(() => Promise.resolve({ data: expectedData, error: null }));
      const selectMock = vi.fn(() => ({ single: singleMock }));
      const insertMock = vi.fn((_data: unknown) => ({ select: selectMock }));
