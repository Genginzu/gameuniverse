...
vi.mock("@/lib/services/igdbService", () => ({
  IGDBService: {
    buildImageUrl: vi.fn(
      (id: string, size: string) => `https://images.igdb.com/igdb/image/upload/t_${size}/${id}.jpg`
    ),
  },
}));
...
describe("transformIGDBGameData", () => {
  ...
  it("transforms background URL from artworks", () => {
    ...
    expect(result.backgroundUrl).toBe(
      "https://images.igdb.com/igdb/image/upload/t_1080p/ar5678.jpg"
    );
  });
  ...
  it("falls back to screenshots for background when no artworks", () => {
    ...
    const result = transformIGDBGameData(game);
    expect(result.backgroundUrl).toBe(
      "https://images.igdb.com/igdb/image/upload/t_1080p/sc9999.jpg"
    );
  });
