...
import {
  adminGenderFormSchema,
  genderTranslationSchema,
} from "@/lib/validations/admin-gender-form";
 ...
 *
 * _For any_ valid gender payload containing a slug and translations,
...
const validGenderGenerator = fc.record({
...
describe("Admin Gender Form Schema - Property-Based Tests", () => {
  describe("Property 1: Gender CRUD round-trip (validation part)", () => {
    it("accepts all valid gender payloads (valid slug + at least one translation)", () => {
      fc.assert(
        fc.property(validGenderGenerator, (data) => {
          const result = adminGenderFormSchema.safeParse(data);
    ...
    it("accepts valid translations with non-empty language_code and name (1-255 chars)", () => {
      fc.assert(
        fc.property(validTranslationGenerator, (translation) => {
          const result = genderTranslationSchema.safeParse(translation);
          expect(result.success).toBe(true);
        }),
        { numRuns: 100 }
      );
  ...
  describe("Property 3: API validation rejects invalid data (validation part)", () => {
      ...
      it("rejects empty slug or slug < 2 chars", () => {
        fc.assert(
          fc.property(tooShortSlugGenerator, (slug) => {
            const result = adminGenderFormSchema.safeParse({
              slug,
    ...
    describe("Missing required fields rejection", () => {
      it("rejects payload with missing slug", () => {
        fc.assert(
          fc.property(
            ...
            (translations) => {
              const result = adminGenderFormSchema.safeParse({ translations });
              expect(result.success).toBe(false);
            }
          ),
          { numRuns: 100 }
      ...
      it("rejects payload with missing translations", () => {
        fc.assert(
          fc.property(validSlugGenerator, (slug) => {
            const result = adminGenderFormSchema.safeParse({ slug });
            expect(result.success).toBe(false);
          }),
          { numRuns: 100 }
        );
