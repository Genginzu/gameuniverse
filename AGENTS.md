...
This is the single, tool-agnostic source of truth for agent behaviour and project
knowledge. `CLAUDE.md` is a symlink to this file. Shared config, skills, steering,
and specs live in `.agents/`; `.claude/` and `.kiro/` are symlinks to `.agents/`.
...
- Identifiers, comments and test titles MUST be written in English.
- The conversation follows the user — French or English.
...
bun run check:agent-symlinks  # Verify .agents/ symlinks + steering cross-refs
...
`.agents/steering/project-architecture.md` for the full hook/service inventory.
...
in `.agents/steering/igdb-field-tracking.md`.
...
- **Styling**: glassmorphism design system — use `.glass-*` utility classes from
  ...
  `.agents/steering/design-glassmorphism.md`.
...
- **Mobile-first**: base classes = mobile, then `sm: md: lg:`. Never `max-md:`
  ...
  `.agents/steering/frontend-mobile-first.md`.
...
- See `.agents/steering/git-auto-commit.md` and
  `.agents/steering/github-workflow.md`.

## Additional steering docs

`.agents/steering/` contains the authoritative conventions. Read the relevant file
...
- `graphify.md` — always-included: use `graphify query` before reading source files

## Claude Code skills

`.agents/skills/` holds on-demand references. They activate via file-path or
...
- **`igdb-api-expert`** — full IGDB API reference (78 endpoints, APICalypse,
  ...
  `scripts/igdb-import/**`. Points at `.agents/skills/igdb-api-expert.md` — read
...
- **`spec-final-validation`** — mandatory tests+lint+build+README block for spec
  `tasks.md`. Activates when writing `.agents/specs/**/tasks.md`.
- **`graphify`** — knowledge graph of the codebase (5867 nodes, 237 communities).
  Use `graphify query "<question>"` for scoped subgraphs instead of reading files.
  Activates on any codebase/architecture question when `graphify-out/graph.json` exists.

## Claude Code hooks

`.agents/settings.json` registers hooks:

**PreToolUse** (before reading/grepping):
- `steering-reminder.sh` (Read|Glob): if `graphify-out/graph.json` exists, forces
  `graphify query` before reading source files. Otherwise, reminds about the
  relevant `.agents/steering/` doc(s) for the file path.
- `graphify-bash-enforce.sh` (Bash): if grep/rg/find/fd is used and a graphify
  graph exists, forces `graphify query` first.

**PostToolUse** (after writing/editing):
- `run-related-tests.sh` (Write|Edit): when a `.ts`/`.tsx` file under `src/` or
...
`permissions` in `.agents/settings.json`.
