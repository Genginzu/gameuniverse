...
- [x] 1. Types et configuration
  - [x] 1.1 Créer `src/types/recommendation.ts` avec les interfaces `GameRecommendation`, `RecommendationsResponse`, `PersonalRecommendationsResponse`, `ScoringWeights`, `CandidateScores`
    - Définir toutes les interfaces du Data Models section du design
    - _Requirements: 5.4, 9.1_

- [x] 2. Fonctions de scoring pures
  - [x] 2.1 Créer `src/lib/services/recommendation/genreScorer.ts`
    - Implémenter `computeGenreScore` avec le coefficient de Jaccard
    - Exporter l'interface `GenreScoreInput`
    - _Requirements: 1.1, 1.2_
  - [x] 2.2 Écrire le property test pour genreScorer
  ...
  - [x] 2.3 Créer `src/lib/services/recommendation/collaborativeScorer.ts`
    - Implémenter `computeCollaborativeScore` avec co-occurrence normalisée
    - Exporter l'interface `CollaborativeScoreInput`
    - _Requirements: 2.1, 2.2, 2.3_
  - [x] 2.4 Écrire le property test pour collaborativeScorer
  ...
  - [x] 2.5 Créer `src/lib/services/recommendation/reviewScorer.ts`
    - Implémenter `computeReviewScore` avec discount de confiance
    - Exporter l'interface `ReviewScoreInput`
    - _Requirements: 3.1, 3.2, 3.3_
  - [x] 2.6 Écrire le property test pour reviewScorer
  ...
  - [x] 2.7 Créer `src/lib/services/recommendation/scoreCombiner.ts`
  ...
  - [x] 2.8 Écrire le property test pour scoreCombiner
  ...
  - [x] 2.9 Créer `src/lib/services/recommendation/index.ts` barrel export
    - Réexporter toutes les fonctions et interfaces de scoring
    - _Requirements: aucun (organisation du code)_

- [x] 3. Checkpoint - Vérifier les fonctions de scoring
  - Ensure all tests pass, ask the user if questions arise.

- [x] 4. Service de recommandation
  - [x] 4.1 Créer `src/lib/services/recommendationService.ts`
  ...
  - [x] 4.2 Écrire les property tests pour le service
  ...
  - [x] 4.3 Écrire les unit tests pour le service
...
- [x] 5. Checkpoint - Vérifier le service
  - Ensure all tests pass, ask the user if questions arise.

- [x] 6. API Routes
  - [x] 6.1 Créer `src/app/api/games/[slug]/recommendations/route.ts`
  ...
  - [x] 6.2 Créer `src/app/api/recommendations/personal/route.ts`
  ...
  - [x] 6.3 Écrire les unit tests pour les API routes
...
- [x] 7. Composants UI
  - [x] 7.1 Créer `src/hooks/useRecommendations.ts`
    - Hook pour fetch les recommandations d'un jeu via l'API
    - Gérer les états loading, error, data
    - _Requirements: 6.1, 6.2_
  - [x] 7.2 Créer `src/components/games/RecommendationSection.tsx`
  ...
  - [x] 7.3 Intégrer `RecommendationSection` dans `GameDetailsContent.tsx`
    - Ajouter la section après les tabs existants
    - Passer le game ID et la locale
    - _Requirements: 6.1_
  - [x] 7.4 Écrire les unit tests pour le hook et le composant
...
- [x] 8. Recommandations personnalisées UI
  - [x] 8.1 Créer `src/hooks/usePersonalRecommendations.ts`
    - Hook pour fetch les recommandations personnalisées via l'API
    - _Requirements: 7.1_
  - [x] 8.2 Créer `src/components/games/PersonalRecommendationSection.tsx`
    - Section pour le profil joueur avec message si bibliothèque vide
    - Réutiliser `GameCard`
    - _Requirements: 7.1, 7.2, 7.3_
  - [x] 8.3 Intégrer dans la page profil du joueur
    - Ajouter la section de recommandations personnalisées
    - _Requirements: 7.1_

- [x] 9. Final checkpoint - Vérifier l'ensemble
...
- [x] 12. README de la fonctionnalité
  - [x] 12.1 Créer `docs/README_GAME_RECOMMENDATIONS.md`
