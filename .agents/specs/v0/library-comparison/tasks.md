...
- [x] 1. Migration SQL et fonction de comparaison
  - [x] 1.1 Créer la migration `supabase/migrations/20240225000001_library_comparison.sql`
...
- [x] 2. Types et service de comparaison
  - [x] 2.1 Ajouter les types `CommonGame` et `CommonGamesResult` dans `src/types/player.ts`
    - `CommonGame` : gameId, slug, title, coverImage, genres
    - `CommonGamesResult` : commonGamesCount, commonGames, pagination
    - _Requirements: 1.2, 3.1_

  - [x] 2.2 Créer `src/lib/services/libraryComparisonService.ts`
  ...
  - [x] 2.3 Écrire les tests property-based pour le service
...
- [x] 3. API Route de comparaison
  - [x] 3.1 Créer `src/app/api/players/[id]/common-games/route.ts`
  ...
  - [x] 3.2 Écrire les tests unitaires pour l'API route
  ...
  - [x] 3.3 Écrire le test property-based pour la validation UUID
...
- [x] 4. Checkpoint — Vérifier que la couche données et API fonctionne
  - Ensure all tests pass, ask the user if questions arise.

- [x] 5. Composants React de comparaison
  - [x] 5.1 Créer `src/components/players/CommonGamesIndicator.tsx`
  ...
  - [x] 5.2 Créer `src/components/players/CommonGamesList.tsx`
  ...
  - [x] 5.3 Créer `src/components/players/LibraryComparisonSection.tsx`
  ...
  - [x] 5.4 Intégrer `LibraryComparisonSection` dans `PlayerDetailsContent.tsx`
  ...
  - [x] 5.5 Écrire le test property-based pour la visibilité de l'indicateur
  ...
  - [x] 5.6 Écrire le test property-based pour la complétude des données
...
- [x] 6. Traductions i18n
  - [x] 6.1 Ajouter les clés de traduction dans `src/messages/fr.json` et `src/messages/en.json`
    - Clés sous `players.comparison` : `commonGames`, `noCommonGames`, `gamesInCommon`, `loading`, `error`, `retry`
    - _Requirements: 6.1, 6.2_

- [x] 7. Checkpoint final — Vérifier l'ensemble
  - Ensure all tests pass, ask the user if questions arise.
...
- [x] 10. README de la fonctionnalité
  - [x] 10.1 Créer `docs/README_LIBRARY_COMPARISON.md`
