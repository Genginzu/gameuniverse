...
Run the block for the current platform. The team is split between Windows and macOS/Linux, so
both are maintained — keep them in sync when either changes.
...
# Stop here rather than writing an empty interpreter path — every later step
# would then run "" and fail with a shell error that says nothing useful.
if (-not $GRAPHIFY_PYTHON) {
    Write-Error "graphify not found and install failed — try: uv tool install graphifyy"
    exit 1
}
...
find_graphify_python() {
    local candidate dir
    # 1. uv tool install — 'uv tool dir' is authoritative, respects UV_TOOL_DIR
    if command -v uv >/dev/null 2>&1; then
        dir=$(uv tool dir 2>/dev/null)
        candidate="$dir/graphifyy/bin/python"
    ...
    if command -v pipx >/dev/null 2>&1; then
        dir=$(pipx environment --value PIPX_LOCAL_VENVS 2>/dev/null)
        candidate="$dir/graphifyy/bin/python"
        if [ -x "$candidate" ] && "$candidate" -c "import graphify" 2>/dev/null; then
            printf '%s' "$candidate"; return 0
        fi
    ...
    if command -v python3 >/dev/null 2>&1 && python3 -c "import graphify" 2>/dev/null; then
        python3 -c "import sys; sys.stdout.write(sys.executable)"; return 0
    fi
    return 1
}

# Try to find the right Python (uv → pipx → active env), install and retry if absent
GRAPHIFY_PYTHON=$(find_graphify_python) || {
    if command -v uv >/dev/null 2>&1; then
        uv tool install --upgrade graphifyy -q 2>&1 | tail -3
    else
        pip install graphifyy -q 2>&1 | tail -3
    fi
    GRAPHIFY_PYTHON=$(find_graphify_python)
}
...
if [ -z "$GRAPHIFY_PYTHON" ]; then
    echo "graphify not found and install failed — try: uv tool install graphifyy" >&2
    exit 1
fi
...
# Save scan root so `graphify update` (no args) knows where to look next time
printf '%s' "$(cd INPUT_PATH && pwd)" > graphify-out/.graphify_root
```

If the import succeeds, print nothing and move straight to Step 2.

**In every subsequent block, run Python through the saved interpreter, never a bare `python3`, so
every step uses the interpreter that actually has graphify:**

- PowerShell: `& (Get-Content graphify-out\.graphify_python)`
- bash: `"$(cat graphify-out/.graphify_python)"`

The remaining steps are written in bash. On Windows, translate them the same way as Step 1 —
the commands and flags are identical, only the shell syntax and the path separator change.
