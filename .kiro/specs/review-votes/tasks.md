...
- [x] 1. Créer la migration Supabase pour la table `review_votes`
  - Créer `supabase/migrations/20240224000001_review_votes.sql`
...
- [ ] 2. Types et fonctions pures
  - [x] 2.1 Étendre les types dans `src/types/review.ts`
    - Ajouter `VoteType`, `ReviewVoteCounts`, `ReviewWithVotes`
    - Mettre à jour `ReviewsResponse` pour utiliser `ReviewWithVotes[]`
    - _Requirements: 2.1, 5.5_

  - [x] 2.2 Créer les fonctions utilitaires pures dans `src/lib/utils/reviewVotes.ts`
...
- [ ] 4. API route pour les votes
  ...
  - [x] 4.2 Étendre GET `/api/reviews` pour inclure les données de vote
  ...
  - [x] 4.3 Écrire le test property-based pour la complétude de la réponse GET
...
- [ ] 5. Service client et hook
  ...
  - [x] 5.3 Écrire le test property-based pour la prévention du self-vote
    - Créer `test/unit/hooks/useReviewVote.property.test.ts`
    - **Property 2: Prévention du vote sur son propre avis**
    - **Validates: Requirements 3.2**

  - [x] 5.4 Écrire les tests unitaires pour le hook
...
- [ ] 7. Composant UI et intégration
  - [x] 7.1 Créer `src/components/games/reviews/ReviewVoteButtons.tsx`
  ...
  - [x] 7.2 Intégrer `ReviewVoteButtons` dans `ReviewCard.tsx`
