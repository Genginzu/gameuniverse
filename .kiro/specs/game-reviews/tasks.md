...
- [ ] 1. Créer la migration Supabase et les types
  - [ ] 1.1 Créer la migration SQL `game_reviews` avec contraintes et RLS
  ...
  - [ ] 1.2 Créer les types TypeScript dans `src/types/review.ts`
    - Interfaces : Review, ReviewFormData, ReviewsResponse
    - _Requirements: 5.2_

- [ ] 2. Créer le schéma de validation et l'utilitaire stripHtmlTags
  - [ ] 2.1 Créer `src/lib/validations/review.ts` avec le schéma Zod
  ...
  - [ ] 2.2 Écrire les property tests pour la validation du rating
    - **Property 1: Validation du rating — valeurs invalides rejetées**
    - **Validates: Requirements 2.2, 2.3**
  - [ ] 2.3 Écrire les property tests pour la validation du contenu HTML
    - **Property 2: Contenu HTML vide rejeté**
    - **Property 3: Contenu HTML trop long rejeté**
    - **Validates: Requirements 3.3, 3.4**
  - [ ] 2.4 Écrire les property tests pour la validation des points
    - **Property 4: Points positifs/négatifs invalides rejetés**
    - **Property 5: Limite du nombre de points respectée**
    - **Validates: Requirements 4.3, 4.4, 4.5**
  - [ ] 2.5 Écrire le property test round-trip pour stripHtmlTags
    - **Property 9: Round-trip stripHtmlTags**
    - **Validates: Requirements 3.3, 3.4**

- [ ] 3. Checkpoint — Vérifier que la validation fonctionne
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 4. Créer la route API reviews
  - [ ] 4.1 Créer `src/app/api/reviews/route.ts` avec GET et POST
  ...
  - [ ] 4.2 Écrire le property test pour le tri des reviews
    - **Property 7: Tri des reviews par date décroissante**
    - **Validates: Requirements 5.1**
  - [ ] 4.3 Écrire le property test pour le calcul de la note moyenne
    - **Property 8: Calcul correct de la note moyenne**
    - **Validates: Requirements 5.4**

- [ ] 5. Créer le service et le hook reviews
  - [ ] 5.1 Créer `src/lib/services/reviewService.ts`
    - Méthodes : `fetchReviews(gameId)`, `submitReview(gameId, data)`
    - _Requirements: 1.2, 5.1_
  - [ ] 5.2 Créer `src/hooks/useReviews.ts`
    - État : reviews, averageRating, loading, error, userHasReviewed
    - Actions : fetchReviews, submitReview avec gestion optimiste
    - _Requirements: 1.2, 5.1, 5.4_

- [ ] 6. Checkpoint — Vérifier que l'API et le service fonctionnent
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 7. Créer les composants UI de base
  - [ ] 7.1 Installer Tiptap et créer
  ...
  - [ ] 7.2 Créer `src/components/games/reviews/RatingInput.tsx`
    - Input numérique avec bornes 0-20, feedback visuel
    - _Requirements: 2.1_
  - [ ] 7.3 Créer `src/components/games/reviews/ReviewPointsList.tsx`
    - Liste dynamique d'inputs pour ajouter/supprimer des points
    - Props : type ("positive" | "negative"), maxPoints, onChange
    - _Requirements: 4.1, 4.2, 4.3, 4.4, 4.5_

- [ ] 8. Créer les composants d'affichage et le formulaire
  - [ ] 8.1 Créer `src/components/games/reviews/ReviewCard.tsx`
    - Affichage d'une review : nom joueur, avatar, note, contenu enrichi, points
      +/-
    - _Requirements: 5.2, 3.5_
  - [ ] 8.2 Créer `src/components/games/reviews/ReviewForm.tsx`
  ...
  - [ ] 8.3 Créer `src/components/games/reviews/ReviewList.tsx`
    - Liste des ReviewCard, état vide avec message d'invitation
    - _Requirements: 5.1, 5.2, 5.3_

- [ ] 9. Intégrer dans l'onglet Avis existant
  - [ ] 9.1 Créer `src/components/games/reviews/GameReviewsTab.tsx`
  ...
  - [ ] 9.2 Modifier `src/components/games/details/GameDetailsTabs.tsx`
    - Remplacer le placeholder "coming soon" de l'onglet reviews par
      `GameReviewsTab`
    - Passer `game.id` et `game.title` en props
    - _Requirements: 1.1, 5.1_
  - [ ] 9.3 Écrire les tests unitaires pour les composants reviews
...
- [ ] 10. Ajouter les traductions i18n
...
- [ ] 12. Exécution des tests complets
  - [ ] 12.1 Exécuter `bun run test:all`
  - [ ] 12.2 Vérifier que tous les tests passent (parallèles + isolés)
  - [ ] 12.3 Corriger les tests en échec si nécessaire

- [ ] 13. Lint du code
  - [ ] 13.1 Exécuter `bun run lint`
  - [ ] 13.2 Vérifier qu'il n'y a pas d'erreurs de lint
  - [ ] 13.3 Corriger les erreurs de lint si nécessaire

- [ ] 14. Build de production
  - [ ] 14.1 Exécuter `bun run build`
  - [ ] 14.2 Vérifier qu'il n'y a pas d'erreurs de compilation
  - [ ] 14.3 Corriger les erreurs de build si nécessaire
