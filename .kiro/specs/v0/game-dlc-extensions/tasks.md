...
- [x] 1. Créer la table `game_dlc_extensions` et les types
  - [x] 1.1 Créer la migration Supabase `supabase/migrations/20240301000001_game_dlc_extensions.sql`
  ...
  - [x] 1.2 Ajouter les types IGDB dans `src/types/igdb.ts`
  ...
  - [x] 1.3 Ajouter les types applicatifs dans `src/types/game.ts`
...
- [x] 2. Implémenter le service IGDB et la logique de transformation
  - [x] 2.1 Modifier `IGDBService.getGameDetails()` dans `src/lib/services/igdbService.ts`
    - Ajouter `dlcs`, `expansions`, `bundles` aux champs de la requête IGDB
    - _Requirements: 1.1_
  - [x] 2.2 Créer `IGDBService.getDlcExtensions()` dans `src/lib/services/igdbService.ts`
  ...
  - [x] 2.3 Créer une fonction utilitaire de transformation dans `src/lib/utils/dlcExtensionUtils.ts`
  ...
  - [x] 2.4 Écrire le test property-based pour la transformation
  ...
  - [x] 2.5 Écrire le test property-based pour le tri
    - **Property 2: Tri des contenus additionnels par catégorie puis par date**
    - **Validates: Requirements 6.1**
    - Fichier : `test/unit/lib/utils/dlcExtensionUtils.property.test.ts`
  - [x] 2.6 Écrire le test property-based pour le regroupement
    - **Property 3: Regroupement par catégorie**
    - **Validates: Requirements 7.3**
    - Fichier : `test/unit/lib/utils/dlcExtensionUtils.property.test.ts`

- [x] 3. Checkpoint - Vérifier les tests
  - Ensure all tests pass, ask the user if questions arise.

- [x] 4. Intégrer dans le service d'import
  - [x] 4.1 Ajouter `createDlcExtensions()` dans `src/lib/services/gameImportService.ts`
  ...
  - [x] 4.2 Ajouter `updateDlcExtensions()` dans `src/lib/services/gameImportService.ts`
    - Supprime les entrées existantes puis appelle `createDlcExtensions`
    - _Requirements: 3.4, 4.2_
  - [x] 4.3 Appeler `createDlcExtensions` dans `importFromIGDB()` et `updateDlcExtensions` dans `syncWithIGDB()`
    - Ajouter les appels après `createVersions`/`updateVersions` respectivement
    - _Requirements: 4.1, 4.2_

- [x] 5. Exposer via l'API et afficher dans l'UI
  - [x] 5.1 Ajouter la requête `game_dlc_extensions` dans `src/app/api/games/[slug]/route.ts`
  ...
  - [x] 5.2 Créer le composant `src/components/games/GameDlcExtensions.tsx`
  ...
  - [x] 5.3 Ajouter l'onglet `dlcExtensions` dans `src/components/games/details/GameDetailsTabs.tsx`
  ...
  - [x] 5.4 Ajouter les traductions dans `src/messages/fr.json` et `src/messages/en.json`
  ...
  - [x] 5.5 Écrire le test property-based pour le rendu des cartes
    - **Property 4: Complétude du rendu par carte**
    - **Validates: Requirements 7.4**
    - Fichier : `test/unit/components/games/GameDlcExtensions.property.test.ts`

- [x] 6. Intégrer dans le script d'import en masse
  - [x] 6.1 Créer la fonction `importDlcExtensions()` dans `scripts/igdb-import/game-importer.ts`
    - Suit le pattern de `importGameVersions`
    - Support dry-run avec affichage du nombre trouvé
    - _Requirements: 5.1, 5.2_
  - [x] 6.2 Appeler `importDlcExtensions` dans `importGameFromIGDB()`
    - Ajouter l'appel après `importGameVersions`
    - _Requirements: 5.1_

- [x] 7. Checkpoint final - Vérifier tous les tests
  - Ensure all tests pass, ask the user if questions arise.
...
- [x] 10. README de la fonctionnalité
  - [x] 10.1 Créer `docs/README_GAME_DLC_EXTENSIONS.md`
  - [x] 10.2 Documenter ce qui a été implémenté, comment y accéder, les prérequis et l'utilisation
