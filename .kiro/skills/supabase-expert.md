...
description: Patterns Supabase du projet — clients, services, migrations, auth, RLS. Activer quand on travaille sur la DB, les API routes, l'auth ou les migrations.
---
...
- En pratique, **rarement utilisé pour les queries data** — tout passe par SWR → API routes
...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json(handleApiError(error, "Failed to ..."), { status: 500 });
  }
}
```
...
const { data } = await supabase.rpc("get_games_listing", {
  p_locale: locale, p_limit: limit, p_offset: offset,
});
```
