    ...
    test("returns paginated reviews with correct structure", async () => {
      ...
      mockSupabaseFrom = mock((table: string) => {
        if (table === "profiles") {
          return {
            select: mock(() => ({
              in: mock(() => Promise.resolve({ data: sampleProfiles, error: null })),
            })),
          };
        }
        // game_reviews
        return {
          select: mock((_cols: string, opts?: { count?: string; head?: boolean }) => {
            if (opts?.head) return Promise.resolve({ count: 2, error: null });
            return {
              order: mock(() => ({
                range: mock(() => Promise.resolve({ data: rows, error: null })),
              })),
            };
          }),
        };
    ...
    test("returns 500 when count query fails", async () => {
      mockSupabaseFrom = mock(() => ({
        select: mock((_cols: string, opts?: { count?: string; head?: boolean }) => {
          ...
          return {
            order: mock(() => ({ range: mock(() => Promise.resolve({ data: [], error: null })) })),
          };
        }),
      }));
      ...
      mockSupabaseFrom = mock((table: string) => {
        if (table === "profiles") {
          return {
            select: mock(() => ({
              eq: mock(() => ({
                single: mock(() => Promise.resolve({ data: { username: "player1" }, error: null })),
              })),
            })),
          };
        }
        // game_reviews
        callCount++;
        const singleMock = mock(() => Promise.resolve({ data: sampleReviewRow, error: null }));
      ...
      mockSupabaseFrom = mock((table: string) => {
        ...
        if (callCount === 1) {
          ...
          return {
            select: mock(() => ({
              eq: mock(() => ({
                single: mock(() => Promise.resolve({ data: { id: "review-1" }, error: null })),
              })),
            })),
          };
        }
        ...
        return {
          select: mock(() => ({
            eq: mock(() => ({
              single: mock(() => Promise.resolve({ data: updatedRow, error: null })),
            })),
          })),
        };
      });
