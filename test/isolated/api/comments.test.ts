...
describe("Comments API — GET /api/comments", () => {
  beforeEach(() => {
    mockGetUser = mock(() => Promise.resolve({ data: { user: null }, error: null }));
  ...
  test("returns comments list with correct structure", async () => {
    ...
    mockGetUser = mock(() => Promise.resolve({ data: { user: AUTHENTICATED_USER }, error: null }));
    ...
    mockSupabaseFrom = mock((table: string) => {
      ...
      if (table === "profiles") {
        return {
          select: mock(() => ({
            in: mock(() => Promise.resolve({ data: profiles, error: null })),
          })),
        };
      }
      // character_comments
      return {
        select: mock(() => ({
          eq: mock(() => ({
            order: mock(() => Promise.resolve({ data: commentRows, error: null })),
          })),
        })),
      };
    });
  ...
  test("creates comment successfully (Req 1.2)", async () => {
    ...
    mockSupabaseFrom = mock((table: string) => {
      ...
      return {
        insert: mock(() => ({
          select: mock(() => ({
            single: mock(() => Promise.resolve({ data: insertedComment, error: null })),
          })),
        })),
      };
    });
    ...
    mockSupabaseFrom = mock(() => ({
      update: mock(() => ({
        eq: mock(() => ({
          eq: mock(() => ({
            select: mock(() => ({
              single: mock(() => Promise.resolve({ data: updatedComment, error: null })),
            })),
          })),
        })),
      })),
