...
const mockRoles = [
  { id: "r1", slug: "protagonist", characterCount: 20, translations: [{ language_code: "en", name: "Protagonist" }] },
  { id: "r2", slug: "antagonist", characterCount: 10, translations: [{ language_code: "en", name: "Antagonist" }] },
];
const mockPagination = { currentPage: 1, totalPages: 1, totalCount: 2, limit: 20, hasNextPage: false, hasPreviousPage: false };

function successFetch() {
  return vi.fn(() =>
    Promise.resolve({ ok: true, status: 200, json: () => Promise.resolve({ roles: mockRoles, pagination: mockPagination }) })
...
describe("useAdminRoles", () => {
  ...
  it("deletes with force=true and refetches", async () => {
    globalThis.fetch = vi.fn((_url: string, opts?: RequestInit) => {
      ...
      return Promise.resolve({ ok: true, status: 200, json: () => Promise.resolve({ roles: mockRoles, pagination: mockPagination }) });
    }) as unknown as typeof fetch;
    const { result } = renderHook(() => useAdminRoles());
    await waitFor(() => expect(result.current.loading).toBe(false));
    await act(async () => { await result.current.deleteRole("protagonist"); });
  ...
  it("checks role usage from 409", async () => {
    globalThis.fetch = vi.fn((_url: string, opts?: RequestInit) => {
      if (opts?.method === "DELETE") return Promise.resolve({ ok: false, status: 409, json: () => Promise.resolve({ usageCount: 20 }) });
      return Promise.resolve({ ok: true, status: 200, json: () => Promise.resolve({ roles: mockRoles, pagination: mockPagination }) });
    }) as unknown as typeof fetch;
    const { result } = renderHook(() => useAdminRoles());
    await waitFor(() => expect(result.current.loading).toBe(false));
    let count: number | undefined;
    await act(async () => { count = await result.current.checkRoleUsage("protagonist"); });
  ...
  it("refetches with last params", async () => {
    const { result } = renderHook(() => useAdminRoles());
    await waitFor(() => expect(result.current.loading).toBe(false));
    await act(async () => { await result.current.fetchRoles({ page: 2, search: "ant" }); });
