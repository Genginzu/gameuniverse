...
const mockPlatforms = [
  { id: "p1", slug: "pc", iconUrl: null, gameCount: 50, translations: [{ language_code: "en", name: "PC" }] },
  { id: "p2", slug: "ps5", iconUrl: null, gameCount: 30, translations: [{ language_code: "en", name: "PS5" }] },
];
const mockPagination = { currentPage: 1, totalPages: 1, totalCount: 2, limit: 20, hasNextPage: false, hasPreviousPage: false };

function successFetch() {
  return vi.fn(() =>
    Promise.resolve({ ok: true, status: 200, json: () => Promise.resolve({ platforms: mockPlatforms, pagination: mockPagination }) })
...
describe("useAdminPlatforms", () => {
  ...
  it("deletes with force=true and refetches", async () => {
    globalThis.fetch = vi.fn((_url: string, opts?: RequestInit) => {
      ...
      return Promise.resolve({ ok: true, status: 200, json: () => Promise.resolve({ platforms: mockPlatforms, pagination: mockPagination }) });
    }) as unknown as typeof fetch;
    const { result } = renderHook(() => useAdminPlatforms());
    await waitFor(() => expect(result.current.loading).toBe(false));
    await act(async () => { await result.current.deletePlatform("pc"); });
  ...
  it("checks platform usage from 409", async () => {
    globalThis.fetch = vi.fn((_url: string, opts?: RequestInit) => {
      if (opts?.method === "DELETE") return Promise.resolve({ ok: false, status: 409, json: () => Promise.resolve({ usageCount: 50 }) });
      return Promise.resolve({ ok: true, status: 200, json: () => Promise.resolve({ platforms: mockPlatforms, pagination: mockPagination }) });
    }) as unknown as typeof fetch;
    const { result } = renderHook(() => useAdminPlatforms());
    await waitFor(() => expect(result.current.loading).toBe(false));
    let count: number | undefined;
    await act(async () => { count = await result.current.checkPlatformUsage("pc"); });
  ...
  it("refetches with last params", async () => {
    const { result } = renderHook(() => useAdminPlatforms());
    await waitFor(() => expect(result.current.loading).toBe(false));
    await act(async () => { await result.current.fetchPlatforms({ page: 2, search: "ps" }); });
