...
const mockGenders = [
  {
    ...
    slug: "male",
    characterCount: 10,
    translations: [{ language_code: "en", name: "Male" }],
  },
  {
    id: "g2",
    slug: "female",
    characterCount: 8,
    translations: [{ language_code: "en", name: "Female" }],
  },
];
...
function successFetch() {
  return vi.fn(() =>
    Promise.resolve({
      ok: true,
      status: 200,
      json: () => Promise.resolve({ genders: mockGenders, pagination: mockPagination }),
    })
...
describe("useAdminGenders", () => {
  ...
  it("deletes and refetches", async () => {
    ...
    await waitFor(() => expect(result.current.loading).toBe(false));
    await act(async () => {
      await result.current.deleteGender("g1");
    });
  ...
  it("checks gender usage via GET", async () => {
    globalThis.fetch = vi.fn((_url: string, opts?: RequestInit) => {
      if (!opts?.method || opts.method === "GET") {
        ...
        if (url.match(/\/genders\/g1$/))
          return Promise.resolve({
            ok: true,
            status: 200,
            json: () => Promise.resolve({ gender: { characterCount: 10 } }),
          });
        return Promise.resolve({
          ok: true,
          status: 200,
    ...
    await act(async () => {
      count = await result.current.checkGenderUsage("g1");
    });
  ...
  it("refetches with last params", async () => {
    ...
    await waitFor(() => expect(result.current.loading).toBe(false));
    await act(async () => {
      await result.current.fetchGenders({ page: 2, search: "male" });
    });
