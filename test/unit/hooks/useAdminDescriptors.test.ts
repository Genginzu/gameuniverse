...
function successFetch() {
  return vi.fn(() =>
    Promise.resolve({
      ok: true,
      status: 200,
      json: () => Promise.resolve({ descriptors: mockDescriptors }),
    })
...
describe("useAdminDescriptors", () => {
  ...
  it("deletes and refetches", async () => {
    ...
    await waitFor(() => expect(result.current.loading).toBe(false));
    await act(async () => {
      await result.current.deleteDescriptor("d1");
    });
  ...
  it("checks descriptor usage from 409", async () => {
    globalThis.fetch = vi.fn((_url: string, opts?: RequestInit) => {
      if (opts?.method === "DELETE")
        return Promise.resolve({
          ok: false,
          status: 409,
          json: () => Promise.resolve({ usageCount: 3 }),
        });
      return Promise.resolve({
        ok: true,
        status: 200,
    ...
    await act(async () => {
      count = await result.current.checkDescriptorUsage("d1");
    });
    expect(count).toBe(3);
  });
  ...
  it("refetches with last search", async () => {
    ...
    await waitFor(() => expect(result.current.loading).toBe(false));
    await act(async () => {
      await result.current.fetchDescriptors("violence");
    });
