...
function successFetch() {
  return vi.fn(() =>
    Promise.resolve({ ok: true, status: 200, json: () => Promise.resolve({ ratings: mockRatings }) })
...
describe("useAdminRatings", () => {
  ...
  it("deletes and refetches", async () => {
    globalThis.fetch = vi.fn((_url: string, opts?: RequestInit) => {
      ...
      return Promise.resolve({ ok: true, status: 200, json: () => Promise.resolve({ ratings: mockRatings }) });
    }) as unknown as typeof fetch;
    const { result } = renderHook(() => useAdminRatings("rs1"));
    await waitFor(() => expect(result.current.loading).toBe(false));
    await act(async () => { await result.current.deleteRating("r1"); });
  ...
  it("checks rating usage from 409", async () => {
    globalThis.fetch = vi.fn((_url: string, opts?: RequestInit) => {
      if (opts?.method === "DELETE") return Promise.resolve({ ok: false, status: 409, json: () => Promise.resolve({ usageCount: 7 }) });
      return Promise.resolve({ ok: true, status: 200, json: () => Promise.resolve({ ratings: mockRatings }) });
    }) as unknown as typeof fetch;
    const { result } = renderHook(() => useAdminRatings("rs1"));
    await waitFor(() => expect(result.current.loading).toBe(false));
    let count: number | undefined;
    await act(async () => { count = await result.current.checkRatingUsage("r1"); });
    expect(count).toBe(7);
  });
  ...
  it("refetches with last search", async () => {
    const { result } = renderHook(() => useAdminRatings("rs1"));
    await waitFor(() => expect(result.current.loading).toBe(false));
    await act(async () => { await result.current.fetchRatings("teen"); });
