...
function successFetch() {
  return vi.fn(() =>
    Promise.resolve({ ok: true, status: 200, json: () => Promise.resolve({ ratingSystems: mockRatingSystems, pagination: mockPagination }) })
...
describe("useAdminRatingSystems", () => {
  ...
  it("deletes and refetches", async () => {
    globalThis.fetch = vi.fn((_url: string, opts?: RequestInit) => {
      ...
      return Promise.resolve({ ok: true, status: 200, json: () => Promise.resolve({ ratingSystems: mockRatingSystems, pagination: mockPagination }) });
    }) as unknown as typeof fetch;
    const { result } = renderHook(() => useAdminRatingSystems());
    await waitFor(() => expect(result.current.loading).toBe(false));
    await act(async () => { await result.current.deleteRatingSystem("rs1"); });
  ...
  it("checks usage from 409", async () => {
    globalThis.fetch = vi.fn((_url: string, opts?: RequestInit) => {
      if (opts?.method === "DELETE") return Promise.resolve({ ok: false, status: 409, json: () => Promise.resolve({ usageCount: 12 }) });
      return Promise.resolve({ ok: true, status: 200, json: () => Promise.resolve({ ratingSystems: mockRatingSystems, pagination: mockPagination }) });
    }) as unknown as typeof fetch;
    const { result } = renderHook(() => useAdminRatingSystems());
    await waitFor(() => expect(result.current.loading).toBe(false));
    let count: number | undefined;
    await act(async () => { count = await result.current.checkRatingSystemUsage("rs1"); });
  ...
  it("refetches with last params", async () => {
    const { result } = renderHook(() => useAdminRatingSystems());
    await waitFor(() => expect(result.current.loading).toBe(false));
    await act(async () => { await result.current.fetchRatingSystems({ page: 2, search: "pegi" }); });
