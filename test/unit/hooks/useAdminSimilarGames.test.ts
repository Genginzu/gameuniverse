...
const mockSimilarGames = [
  { id: "sg1", similarIgdbId: 100, displayOrder: 1, game: { id: "g1", slug: "zelda", title: "Zelda", coverImage: null } },
  { id: "sg2", similarIgdbId: 200, displayOrder: 2, game: { id: "g2", slug: "mario", title: "Mario", coverImage: null } },
];

function successFetch() {
  return vi.fn((url: string, opts?: RequestInit) => {
    if (opts?.method === "POST") return Promise.resolve({ ok: true, status: 200, json: () => Promise.resolve({ success: true }) });
...
describe("useAdminSimilarGames", () => {
  ...
  it("removes a similar game", async () => {
    ...
    await act(async () => { success = await result.current.removeSimilarGame("sg1"); });
  ...
  it("adds a similar game", async () => {
    ...
    await act(async () => { res = await result.current.addSimilarGame("zelda"); });
    expect(res?.ok).toBe(true);
    const calls = (globalThis.fetch as unknown as ReturnType<typeof vi.fn>).mock.calls;
    const post = calls.filter((c) => (c as unknown[])[1] && ((c as unknown[])[1] as RequestInit).method === "POST");
