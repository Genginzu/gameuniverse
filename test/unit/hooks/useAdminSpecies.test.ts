...
const mockSpecies = [
  { id: "s1", slug: "human", characterCount: 50, translations: [{ language_code: "en", name: "Human" }] },
  { id: "s2", slug: "elf", characterCount: 15, translations: [{ language_code: "en", name: "Elf" }] },
];
const mockPagination = { currentPage: 1, totalPages: 1, totalCount: 2, limit: 20, hasNextPage: false, hasPreviousPage: false };

function successFetch() {
  return vi.fn(() =>
    Promise.resolve({ ok: true, status: 200, json: () => Promise.resolve({ species: mockSpecies, pagination: mockPagination }) })
...
describe("useAdminSpecies", () => {
  ...
  it("deletes and refetches", async () => {
    globalThis.fetch = vi.fn((_url: string, opts?: RequestInit) => {
      ...
      return Promise.resolve({ ok: true, status: 200, json: () => Promise.resolve({ species: mockSpecies, pagination: mockPagination }) });
    }) as unknown as typeof fetch;
    const { result } = renderHook(() => useAdminSpecies());
    await waitFor(() => expect(result.current.loading).toBe(false));
    await act(async () => { await result.current.deleteSpecies("s1"); });
  ...
  it("checks species usage via GET", async () => {
    globalThis.fetch = vi.fn((_url: string, opts?: RequestInit) => {
      if (!opts?.method || opts.method === "GET") {
        ...
        if (url.match(/\/species\/s1$/)) return Promise.resolve({ ok: true, status: 200, json: () => Promise.resolve({ species: { characterCount: 50 } }) });
        return Promise.resolve({ ok: true, status: 200, json: () => Promise.resolve({ species: mockSpecies, pagination: mockPagination }) });
      }
    ...
    await act(async () => { count = await result.current.checkSpeciesUsage("s1"); });
  ...
  it("refetches with last params", async () => {
    const { result } = renderHook(() => useAdminSpecies());
    await waitFor(() => expect(result.current.loading).toBe(false));
    await act(async () => { await result.current.fetchSpecies({ page: 2, search: "elf" }); });
