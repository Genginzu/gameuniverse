...
function successFetch() {
  return vi.fn(() =>
    Promise.resolve({
      ok: true,
      status: 200,
      json: () => Promise.resolve({ reviews: mockReviews, pagination: mockPagination }),
    })
...
describe("useAdminReviews", () => {
  ...
  it("deletes and refetches", async () => {
    ...
    await waitFor(() => expect(result.current.loading).toBe(false));
    await act(async () => {
      await result.current.deleteReview("rv1");
    });
  ...
  it("refetches with last params", async () => {
    ...
    await waitFor(() => expect(result.current.loading).toBe(false));
    await act(async () => {
      await result.current.fetchReviews({ page: 2, search: "amazing" });
    });
