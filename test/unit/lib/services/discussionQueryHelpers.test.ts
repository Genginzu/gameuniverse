...
describe("discussionQueryHelpers", () => {
  it("returns profile map", async () => {
    mockFrom.mockReturnValue(
      chain({ data: [{ id: "u1", username: "alice", avatar_url: null }], error: null })
    );
  ...
  it("returns last messages map", async () => {
    mockFrom.mockReturnValue(
      chain({
        data: [{ conversation_id: "c1", content: "Hi", sender_id: "u1", created_at: "2024-01-01" }],
        error: null,
      })
    );
  ...
  it("returns unread counts", async () => {
    mockFrom.mockReturnValue(
      chain({ data: [{ conversation_id: "c1" }, { conversation_id: "c1" }], error: null })
    );
