...
describe('GenreService', () => {
  it('fetches genres', async () => {
    ...
    vi.mocked(globalThis.fetch).mockResolvedValueOnce({ ok: true, json: () => Promise.resolve({ genres }) } as Response);
    const result = await GenreService.fetchGenres('fr');
    expect(result).toEqual(genres);
  });

  it('handles error', async () => {
    vi.mocked(globalThis.fetch).mockResolvedValueOnce({ ok: false, status: 500, statusText: 'Internal Server Error' } as Response);
