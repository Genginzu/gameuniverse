...
describe('DELETE /api/admin/webhooks/registrations/[id]', () => {
  ...
  test('returns 403 when not admin', async () => {
    ...
    const req = new NextRequest('http://localhost/api/admin/webhooks/registrations/42', { method: 'DELETE' });
  ...
  test('returns 502 when IGDB API fails', async () => {
    (globalThis.fetch as ReturnType<typeof vi.fn>).mockResolvedValue({ ok: false, status: 500, text: async () => 'error' });
    const req = new NextRequest('http://localhost/api/admin/webhooks/registrations/42', { method: 'DELETE' });
    const res = await DELETE(req, { params: Promise.resolve({ id: '42' }) });
