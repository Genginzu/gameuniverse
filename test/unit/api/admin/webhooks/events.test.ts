...
describe('GET /api/admin/webhooks/events', () => {
  ...
  test('returns 200 with events and pagination', async () => {
    const events = [{ id: 'e1', igdb_id: 1, entity_type: 'games', game_id: null, character_id: null, payload: {} }];
  ...
  test('applies filters via query chain', async () => {
    ...
    const req = new NextRequest('http://localhost/api/admin/webhooks/events?entityType=games&status=received');
    const res = await GET(req);
    expect(res.status).toBe(200);
  });
});
