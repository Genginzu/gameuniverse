...
describe('GET /api/admin/webhooks/events/not-imported-ids', () => {
  ...
  test('returns 200 with igdbIds array', async () => {
    mockRange.mockResolvedValue({ data: [{ igdb_id: 10 }, { igdb_id: 20 }, { igdb_id: 10 }], error: null });
    const req = new NextRequest('http://localhost/api/admin/webhooks/events/not-imported-ids?limit=5');
    const res = await GET(req);
