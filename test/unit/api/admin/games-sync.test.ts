...
vi.mock("@/lib/services/igdb-sync", () => ({
  ...
  syncAllGameFields: vi.fn(async () => ({
    success: true,
    syncedFields: ["translations", "cover_image"],
  })),
}));
vi.mock("@/lib/utils/field-tracking", () => ({
  TRACKABLE_FIELDS: ["translations", "cover_image", "release_date"],
}));
...
const makeReq = (body: object) =>
  new NextRequest("http://localhost/api/admin/games/test-id/sync", {
    method: "POST",
