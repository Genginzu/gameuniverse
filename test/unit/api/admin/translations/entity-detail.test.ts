...
describe('GET /api/admin/translations/entity-detail', () => {
  ...
  test('returns 403 when not admin', async () => {
    ...
    const req = new NextRequest(`http://localhost/api/admin/translations/entity-detail?type=games&entityId=${validId}`);
    const res = await GET(req);
    expect(res.status).toBe(403);
  });

  test('returns 400 with invalid params', async () => {
    const req = new NextRequest('http://localhost/api/admin/translations/entity-detail?type=invalid&entityId=not-uuid');
    const res = await GET(req);
