...
describe('GET /api/admin/age-classifications/[id]/descriptors', () => {
  ...
  test('returns 200 with descriptors', async () => {
    ...
    mockFrom.mockImplementation(() => {
      ...
      if (callCount === 2) return chainMock([{ id: 'd1', rating_system_id: 'sys1', code: 'VIOLENCE', icon_url: null, content_descriptor_translations: [] }]);
...
describe('POST /api/admin/age-classifications/[id]/descriptors', () => {
  ...
  test('returns 201 on create', async () => {
    ...
    mockFrom.mockImplementation(() => {
      ...
      if (callCount === 3) return chainMock({ id: 'd-new', rating_system_id: 'sys1', code: 'FEAR', icon_url: null });
      return chainMock(null); // translations insert
    });
    const body = { code: 'FEAR', icon_url: null, translations: [{ language_code: 'fr', name: 'Peur', description: '' }] };
