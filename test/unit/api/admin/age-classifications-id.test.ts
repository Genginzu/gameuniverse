...
describe('GET /api/admin/age-classifications/[id]', () => {
  ...
  test('returns 200 with rating system', async () => {
    const system = { id: 'test-id', code: 'PEGI', name: 'PEGI', description: null, country_codes: [], website_url: null };
...
describe('PUT /api/admin/age-classifications/[id]', () => {
  ...
  test('returns 200 on update', async () => {
    const updated = { id: 'test-id', code: 'PEGI', name: 'PEGI Updated', description: null, country_codes: [], website_url: null };
    let callCount = 0;
    ...
    const req = new NextRequest(url, { method: 'PUT', body: JSON.stringify({ code: 'PEGI', name: 'PEGI Updated' }), headers: { 'Content-Type': 'application/json' } });
