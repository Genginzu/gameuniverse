...
description: Checklist for adding a new synchronizable IGDB field. The change must land in 4+ files consistently (tracking constants, sync map, form labels + i18n, bulk import script). Activate when touching `field-tracking*`, `igdb-sync*`, `GameFormSyncTab*`, `scripts/igdb-import/game-importer*`, or admin-games files that deal with IGDB field synchronization.
---

# Skill: IGDB Field Tracking — Adding a new field

Full procedure: `.kiro/steering/igdb-field-tracking.md`. The manual-edit tracking system uses a fixed list of trackable field categories. Adding a new synchronizable IGDB field requires coordinated updates across the following files.
...
- Add a sync function in `src/lib/services/igdb-sync-fields.ts` mapping IGDB → Supabase shape
...
- Add translations under `admin.games.form` in **both** `src/messages/fr.json` and `src/messages/en.json`

### 4. `scripts/igdb-import/game-importer.ts`

- Update `syncExistingGame()` to include the new field in the override-respecting sync logic
